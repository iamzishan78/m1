...
export default function Contacts(props) {
				...
				let filterData = filteredContacts.map((dataMap, index) => {
					if (dataMap === allContactList?.paginatedContacts?.edges[0].node.name) {
						contactDetail = {
							...
							homePhone: allContactList?.paginatedContacts?.edges[0].node.homePhone
								? allContactList?.paginatedContacts?.edges[0].node.homePhone
								: '',
							mobilePhone: allContactList?.paginatedContacts?.edges[0].node.mobilePhone
								? allContactList?.paginatedContacts?.edges[0].node.mobilePhone
								: '',
							address1: allContactList?.paginatedContacts?.edges[0].node.address1
								? allContactList?.paginatedContacts?.edges[0].node.address1 +
									' ' +
									allContactList?.paginatedContacts?.edges[0].node.city +
									' ' +
									allContactList?.paginatedContacts?.edges[0].node.state +
									' ' +
									allContactList?.paginatedContacts?.edges[0].node.zip
								: '',
							primaryEmail: allContactList?.paginatedContacts?.edges[0].node.primaryEmail
								? allContactList?.paginatedContacts?.edges[0].node.primaryEmail
								: '',
						};
	...
	const DeleteContact = async index => {
		let descriptorId =
			get(stateApp, `activeDeal.contacts[${index}].descriptorId`) || get(stateApp, `activeDeal.contacts[${index}]._id`);
	...
	const gotoContact = index => {
		if (props?.stateAppKey && stateApp[props.stateAppKey]?.contacts?.length) {
			...
			history.push(
				`/contact/details/${stateApp.activeDeal?.contacts[index]?._id}?return-url=${history.location.pathname}`
			);
		}
	};
						...
						filteredContacts.map((c, i) => (
							<>
								<ListItem key={i}>
									{/* {c} */}
									<Accordion
										expanded={expandedPanel === 'panel' + i.toString()}
										onChange={handleAccordionChange('panel' + i.toString())}
									>
										<AccordionSummary
											classes={{
										...
										>
											<ListItemIcon>
												<Avatar
													color={Avatar.getRandomColor(c.name ? c.name : c, [
														'#b5d2f6',
														'#ade2e9',
														'#eaeaea',
														'#f2c1e2',
											...
											{stateApp.activeDeal?.contacts?.length &&
										...
										<AccordionDetails className={classes.accDetail}>
											<div className="acc-data">
												{c.homePhone ? (
													<p className="address">
														<CallOutlinedIcon /> <Typography className="address_tabs">{c.homePhone}</Typography>
													</p>
												) : null}
												{c.mobilePhone ? (
													<p className="address">
														<PhoneIphoneIcon /> <Typography className="address_tabs">{c.mobilePhone}</Typography>
													</p>
												) : null}
												{c.primaryEmail ? (
													<p className="address">
														<EmailOutlinedIcon /> <Typography className="address_tabs"> {c.primaryEmail}</Typography>
													</p>
												) : null}
												{c.address1 ? (
													<p className="address">
														<DomainOutlinedIcon /> <Typography className="address_tabs">{c.address1}</Typography>
													</p>
												) : null}
											</div>
										</AccordionDetails>
