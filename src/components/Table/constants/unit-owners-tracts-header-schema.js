...
const getHeaders = ({interestMapping, layerType}) => {
  const UnitOwnersTractHeadCells = [
    ...
    // ...(interestMapping?.['Lease Royalty Interest']?.includes(layerType) ? [{ name: "lease_royalty_interest", esKey: "lease_royalty_interest", type: "number", label: "LRI", options: { filter: true } }] : []),
