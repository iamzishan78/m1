...
const getHeaders = ({ interestMapping, layerType, isTestcase }) => {
	const UnitOwnersTractHeadCells = [
		...
		{ name: 'nra', esKey: 'nra', label: 'NRA', type: 'number', options: { filter: true } },
		{
			...
			esKey: 'acquisition_nra',
			label: 'Acquisition $/NRA',
			type: 'number',
			options: { editable: false, filter: true, customRender: value => <p>{vf_currency_to_fixed(value, 2)}</p> },
		},
		{
			name: 'acquisition_cost',
			esKey: 'acquisition_cost',
			label: 'Acquisition Cost',
			type: 'number',
			options: { filter: true, customRender: value => <p>{vf_currency_to_fixed(value, 2)}</p> },
		},
		{
			name: 'depthFrom',
			label: 'Depth From',
		...
		{
			name: 'department',
			esKey: 'tract.department.keyword',
			label: 'Department',
			editable: true,
			options: { filter: true },
		},
