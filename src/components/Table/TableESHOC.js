...
export const TableESHOC = Component => {
	const HocWithDefaultProps = function HOC(props) {
		...
		const getNonGridViewColumnsData = (cols = columns, metaDatas = metaDataRef.current) => {
			if (cols?.length > 0 && metaDatas) {
				let filterColumns = cols.filter(
					col => !col._id && !props.actionColumns.includes(col.label) && !props.actionColumns.includes(col.name)
				);
				let actionColumns = cols.filter(
					col => props.actionColumns.includes(col.label) || props.actionColumns.includes(col.name)
				);
				...
				metaDatas = metaDatas
					.filter(meta => !cols.find(col => col.name === meta.name) && meta.isControlled !== true)
					.map(meta => ({
						name: meta.name,
						label: meta.label,
						esKey: meta.esKey,
					...
					client
						.query({
							...
							variables: {
								...
								category: tableMeta?.typeKeyword?.metaModule,
								isControlled: false,
							},
						})
		...
		const addColumnOptions = tableCols => {
			...
			tableCols.forEach((column, index) => {
				...
				/// apply global settings unless ignored
				if (
					column?.options?.ignoreGlobal ||
					props.actionColumns.includes(column.label) ||
					props.actionColumns.includes(column.name)
				) {
					column.options = {
						...column.options,
					};
						...
						customFilterListOptions: {
							render: v =>
								v.map(l => {
									...
									return l === 'true' && column?.options?.forceFilter
										? 'Yes'
		...
		const initializeTableActions = (tableState, meta, tableData, columns, gqlQuery, selectedGridView = {}) => {
			...
			// temporary patch
			if (
				tableState?.filterList?.[2]?.includes('Expiration') ||
				tableState?.filterList?.[2]?.includes('Option to Extend')
			) {
				...
				if (val.length > 0 && columns[index]) {
					if (columns[index].custom?.isDate || columns[index].custom?.isDateTime) {
						...
						if (filterData) {
							...
							pageESVariables.variables.filters.push({
								field: columns[index].esKey,
								value: data.key_as_string,
								oRFilter,
							});
						}
					...
					gqlQuery({
						...
						variables: {
							...
							pagination: {
								...
								after: isFiniteScroll
									? afterSort
									: rows && tableState.page > meta.pageInd
										? rows[rows.length - 1]?.sort
										: null,
							},
							filters: handleMultiFieldFilter(pageESVariables.variables.filters.concat(tableMeta.filters)),
						},
		...
		const updateGridViewRedux = useCallback(
			tableState => {
		...
		const onDownload = async () => {
			if (tableMeta?.datasets) {
				...
				const selectedIds =
					selectedRowsValues && selectedRowsValues.length > 0 ? selectedRowsValues.map(item => item._id) : null;
				...
				let filteredColumns = columns.filter(
					c =>
						((c?.options?.display !== false && c?.options?.display !== 'false') || c?.options?.download) &&
						c?.label !== ' '
				);
				...
				const csvData = getCSVData(
					hits,
					tableStateRef.current.columns.filter(
						c => (c?.display !== false && c?.display !== 'false' && c.label !== ' ') || c?.download
					)
				);
		...
		const onTableChange = async (action, tableState, rows, meta) => {
			...
			activeFiltersRef.current = handleMultiFieldFilter(
				existingFilter.concat(tableActions.pageESVariables.variables.filters)
			);
			...
			switch (action) {
				...
				case 'rowSelectionChange':
					let allRows = [];
					if (
						(tableMeta.isSelectedAllAllowed && tableState.selectedRows.data.length === tableState.data.length) ||
						tableState.selectedRows.data.length > tableState.data.length
					) {
						...
						for (let i = 0; i < total; i++) {
							rowsSelected.push(isSelectAll ? i : tableState.selectedRows.data[i].index);
						}
		...
		const viewColumnsChange = tableColumns => {
			...
			for (let i = 0; i < tableColumns.length; i++) {
				if (cols[i]) {
					if (
						(tableColumns[i].display === 'true' || tableColumns[i].display === true) &&
						tableColumns[i].display !== false
					) {
	...
	function HOCContainer(props) {
		...
		const stateAppMemo = useMemo(
			() => ({ user: stateApp.user, filtersData: stateApp.filtersData }),
			[stateApp.filtersData, stateApp.user]
		);
