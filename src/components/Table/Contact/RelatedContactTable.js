...
const CustomChip = ({ label, onDelete }) => {
	if (['Expiration', 'Option to Extend'].includes(label)) return null;
...
function RelatedContactsTable(props) {
	...
	useEffect(() => {
		props.setTableMeta({
			filters: [...getFilters([{ field: 'relatedContacts.relatedObject', value: props.contactId }])],
			searchFields: ['name', 'mobilePhone', 'homePhone', 'address1', 'relatedContacts.relationshipType'],
		...
		props.filterToggle,
		props.activityFilterByType,
		props.activityFilterByTime,
		props.activityFilterByOwner,
	]);
			...
			<Table
				...
				options={{
					...
					onRowClick: (rowData, { dataIndex, rowIndex }) => {
						props.setDrawer && props.setDrawer('agrmt-existing'); // add check Only call the function if it passes
