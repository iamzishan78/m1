...
const interestKeys = ['working_interest', 'royalty_interest', 'orri', 'nri', 'nra'];
		...
		return hits.map(hit => {
			...
			Object.keys(hit).forEach(key => {
				if (interestKeys.includes(key)) {
					if (typeof hit[key] === 'number') hit[key] = addTrailingZeros(hit[key]);
			...
			hit = props.setGenricData(hit, hit?.contact?._id, genericDataActions, genericDataActions);
			return hit;
		});
	};
				...
				>
					...
					>
						{(!props.selectedRows || props.selectedRows?.length === 0) && (
							<CircularProgress size={40} color="secondary" style={{ marginRight: '1em' }} />
						)}
						{props.selectedRows && props.selectedRows?.length !== 0 && (
							<>
								<Button
	...
	return (
		<Container maxWidth={false} className={classes.container} id={props.id ? props.id : props.parent}>
			...
			{openCustomDialog === 'buyContactsInfoData' && (
				<RightDialog open={true} handleClickDialogClose={() => setOpenCustomDialog('')} width={'700px'}>
			...
			{openCustomDialog === 'recalculate' && (
				<RecalculateSlideout onClose={() => setOpenCustomDialog('')} rows={selectedRows} setRows={setSelectedRows} />
			)}
