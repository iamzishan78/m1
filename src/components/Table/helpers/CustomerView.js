...
const CustomerViewCol = props => {
	...
	const setItems = newState => {
		setStateIfDeepEqual(Items, newState);
	};
		...
		<>
			<div className={classes.container} id="customViewColumns">
				...
				<div>
					...
					>
						<AddIcon className={classes.addIcon} /> <span className={classes.f13}>Add field</span>
					</div>
				</div>
					...
					<SortableComponent
						...
						setItems={value => {
							...
							const stickyColumns = columns.filter(
								column => column.setCellProps && findInFunction('sticky', column.setCellProps)
							);
						...
						{tableColumns.find(co => co.name === item.name)?.isCustom && (
							<IconButton
								...
								onClick={() => {
									...
									setStateApp(stateApp => ({
										...stateApp,
										selectedMeta: tableColumns.find(co => co.name === item.name),
										showFieldModal: true,
									}));
								}}
							>
