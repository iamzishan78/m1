...
export default function FilterAbstract({ keys }) {
	...
	const [abstractName, setAbstractName] = useState(stateNav.abstractName ? stateNav.abstractName : []);
	const [abstractList, setAbstractList] = useState();
	const [queryAbstract, { loading, data }] = useQueryAbstractBySurvey(stateNav.surveyName);
