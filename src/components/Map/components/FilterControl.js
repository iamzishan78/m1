...
function FilterControl() {
	...
	const [bulkTrackAllUntrackAllWells, { data: dataWells }] = useMutation(BULKTRACKALLUNTRACKALL);
	const [bulkTrackAllUntrackAllOwners, { data: dataOwners }] = useMutation(BULKTRACKALLUNTRACKALL);

	const [getWellsOwners, { data: dataWellsOwners }] = useLazyQuery(WELLSOWNERSQUERY);

	/// success wells tracked
	useEffect(() => {
		if (dataWells) {
			if (dataWells.bulkTrackAllUntrackAll && dataWells.bulkTrackAllUntrackAll.success) {
	...
	useEffect(() => {
		if (dataOwners) {
			if (dataOwners.bulkTrackAllUntrackAll && dataOwners.bulkTrackAllUntrackAll.success) {
				dispatch(showSuccessMessage('The wells owners are successfully tracked'));
	...
	return (
		<>
			<StyledIconButton onClick={toggleOpenContorl} aria-label="toggle" aria-describedby={id}>
				{!openedControl && <MenuIcon fontSize="small" style={{ margin: '5px', padding: '2px' }} />}
			</StyledIconButton>
			...
			>
				...
				<Tooltip title={!showTrack ? 'Please zoom in' : ''} placement="top-start">
					<span>
						<StyledListItem2 button onClick={handleOpenTrack} disabled={!showTrack}>
							<ListItemIcon>
				...
				<Collapse in={openTrack && showTrack} timeout="auto" unmountOnExit>
					<List disablePadding>
						<Tooltip title={!showTrack ? 'Please zoom in' : ''} placement="top">
							<StyledListItem ContainerComponent="li" onClick={handleTrackWells} disabled={!showTrack}>
						...
						<Tooltip title={!showTrack ? 'Please zoom in' : ''} placement="top">
							<StyledListItem ContainerComponent="li" onClick={handleTrackOwners} disabled={!showTrack}>
