...
SRMode.computeAxes = function (state, polygon) {
	...
	if (polygon.geometry.type === Constants.geojsonTypes.POLYGON) corners = polygon.geometry.coordinates[0].slice(0);
		...
		let temp = [];
		// let history = {}
		// corners = polygon.geometry.coordinates[0][0].slice(0)
		temp.push(polygon.geometry.coordinates[0][0][2]);
		temp.push(polygon.geometry.coordinates[15][0][3]);
		temp.push(polygon.geometry.coordinates[12][0][2]);
		temp.push(polygon.geometry.coordinates[3][0][0]);
		temp.push(polygon.geometry.coordinates[0][0][2]);
		sCenters = { '0.0.2': 0, '15.0.3': 1, '12.0.2': 2, '3.0.0': 3 };
		// polygon.geometry.coordinates.forEach((c, index) => {
		//     c.forEach((c2, index2) => {
		//         c2.forEach((c3, index3) => {
		//             sCenters[`${index}.${index2}.${index3}`] = c3
		//             // if (!history[c3[0]] && !history[c3[1]]) {
		//             //     temp.push(c3);
		//             //     history[c3[0]] = c3[0]
		//             //     history[c3[1]] = c3[1]
		//             // }
...
SRMode.dragScalePoint = function (state, e, delta) {
	...
	if (state?.scaling?.sCenters && typeof state?.scaling?.sCenters[path] !== undefined) {
		cIdx = state.scaling.sCenters[path];
	}
	// TODO validate cIdx
