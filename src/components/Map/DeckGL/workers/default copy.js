export const workerFunction = event => {
  ...
  function geojsonToFlatGeojson(
    features,
    options = { coordLength: 2, fixRingWinding: true }
  ) {
  ...
  function deduceArrayType(x, constructor) {
    ...
    return constructor === Float64Array || Math.fround(x) !== x
      ? Float64Array
      : Float32Array;
  }
  ...
  function triangulatePolygon(
    ...
    areas,
    indices,
    { startPosition, endPosition, coordLength }
  ) {
  ...
  function handlePolygon(geometry, polygons, indexMap, coordLength, properties) {
    ...
    for (let l = 0, ll = geometry.indices.length; l < ll; ++l) {
      ...
      for (let i = 0, il = indices.length; i < il; ++i) {
        ...
        polygons.primitivePolygonIndices[indexMap.polygonRing++] =
          indexMap.polygonPosition;
  ...
  function flatGeojsonToBinary(features, geometryInfo, options) {
    ...
    const numericPropKeys = Object.keys(propArrayTypes).filter(
      k => propArrayTypes[k] !== Array
    );
    return fillArrays(
      features,
      {
  ...
  function geojsonToBinary(
    features,
    options = { fixRingWinding: true, triangulate: true }
  ) {
