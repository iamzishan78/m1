...
const onWellClick = (object, layerId) => {
	...
	if (!booleanPointInPolygon(point(feature.geometry.coordinates), polygon(currentPolygon.geometry.coordinates))) {
	...
	if (feature) {
		drawWellBoundary([feature?.geometry?.coordinates[0], feature?.geometry?.coordinates[1]]);
