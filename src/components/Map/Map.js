...
function Map({
	...
	async function getCustomLayer() {
		...
		if (type === 'wells') {
			...
			if (map && currentFeature) {
				...
				if (currentFeature.Longitude) drawWellBoundary(map, [currentFeature.Longitude, currentFeature.Latitude]);
			}
			return;
		}
		...
		if (layer?.customLayer) {
			...
			if (!oneTimeMapBounds) {
				...
				setOneTimeMapBounds({
					...
					options: {
						easing: () => 1,
						padding: layerPadding ? layerPadding : { top: 200, bottom: 200, left: 1200, right: 0, linear: true },
					},
				});
			}
	...
	useEffect(() => {
		if (hookState.layers.get().length === 0 && stateApp?.layers?.length > 0) hookState.layers.set(stateApp.layers);
	}, [stateApp.layers]);
	...
	useEffect(() => {
		if (dataTracks && dataTracks.tracksByObjectType) {
			if (dataTracks.tracksByObjectType.length !== 0) {
				const tracksIdArray = dataTracks.tracksByObjectType.map(track => track.trackOn);
	...
	useEffect(() => {
		if (dataTracksOwner && dataTracksOwner.tracksByObjectType) {
			if (dataTracksOwner.tracksByObjectType.length !== 0) {
				var objectsIdsArray = dataTracksOwner.tracksByObjectType.map(item => item.trackOn);
				getOwners({
					variables: {
	...
	const [getRecentPermitDetail, { loading: loadingPermitSummary, data: dataPermitSummary }] = useLazyQuery(
		PERMITDETAILQUERY,
		{
	...
	useEffect(() => {
		if (viewFileResult && viewFileResult.viewFile && stateApp.layers) {
			...
			if (result.uri && result.internalKey) {
				...
				dispatch(
					showErrorMessage('The file is not ready yet, please wait a few minutes and then reload the application.')
				);
			}
		}
	}, [viewFileResult]);
	...
	const setLayer = (data, identifier, map, bLayer = null) => {
		...
		for (let i = (paintProps ? paintProps.length : 0) - 1; i >= 0; i--) {
			...
			if (map.getLayer(layerId)) {
				...
				if (config.layerShapeName && config.groupId) {
					if (config.layerGeometry === 'Polygon') {
						layerConfig.filter = [
							'all',
							['==', 'layerShapeName', config.layerShapeName],
							['in', 'layerGeometry', 'Polygon', 'MultiPolygon'],
						];
					} else {
						layerConfig.filter = [
							'all',
							['==', 'layerShapeName', config.layerShapeName],
							['==', 'layerGeometry', config.layerGeometry],
						];
					}
			...
			if (prop.clusterProps) {
				...
				const clusterVisible = visible && mLayer && !mLayer.source.includes('_filter') && !clustersOff;

				const clusterVar = layerId + '-clusters';
				const clusterLabelBar = layerId + '-clusters-counts';
				if (map.getLayer(clusterLabelBar)) {
					map.setLayoutProperty(clusterLabelBar, 'visibility', clusterVisible ? 'visible' : 'none');
				} else {
					map.addLayer({
						id: clusterLabelBar,
						type: 'symbol',
				...
				if (map.getLayer(clusterVar)) {
					map.setLayoutProperty(clusterVar, 'visibility', clusterVisible ? 'visible' : 'none');
	...
	useEffect(() => {
		...
		const udLayerClickHandler = feature => {
			...
			if (ifGenericShapeSource(feature.source)) {
				...
				['interests_source', 'area of interest_source'].includes(feature.source) &&
				!drawMode.includes('draw') &&
				!drawMode.includes('drag')
			) {
				setStateApp(state => {
					if (state.isDrawing) return state;
					state = {
			...
			setStateApp(state => {
				if (
					(!state.showDrawShapesPopup || ifDefaultSources(feature.source)) &&
					state.shapeEditMode !== 'redraw' &&
					state.shapeEditMode !== 'fullEdit'
				)
		...
		const mapClickHandler = async e => {
			...
			hookState.layers.get()?.forEach(layer => {
				const interaction =
					layer.layerSettings.interaction.interactionAble && layer.layerSettings.interaction.interactionDetail.click;
				const visible = layer.layerSettings.showable && layer.layerSettings.visiable !== false;
					...
					if (layer.layerCategory === 'UD layer') {
						layer.layerPaintProps.forEach(paintProps => {
							...
							if (map.getLayer(layerId)) {
								...
								if (ifDefaultLayers(layer.identifier)) {
			...
			if (isNormalClick && features && features.length > 0) {
				...
				switch (true) {
					...
					case layerId === 'wellpoints' ||
						...
						layerId === 'Tags Filter' ||
						((layerId === 'Search' || layerId === 'Search-line') && !hasSearchCluster) ||
		...
		const mapRightClickHandler = e => {
			...
			const fileLayers = hookState.layers
				.get()
				.filter(layer => layer.layerType === 'file layer')
			...
			for (let i = 0; i < hookStateAppLayers.length; i++) {
				...
				if (layer.layerType === 'vector layer') {
					...
					if (layer.layerPaintProps && layer.layerPaintProps.length > 0) {
						...
						if (data) {
							if (data.length > 0)
								if (data[0].__typename === 'CustomLayer') data = data.filter(layer => layer?.shapeJson?.geometry?.type);
	...
	useEffect(() => {
		...
		if (map) {
			...
			if (stateNav.prodOptions) {
				productionFilterCount += stateNav.prodOptions.length;
			}
			...
			if (stateNav.filterTags && stateNav.filterTags.length > 0) {
				...
				if (stateApp.wellListFromTagsFilter && stateApp.wellListFromTagsFilter.length > 0) {
				...
				filterLayers.forEach(filterLayer => {
					if (
						...
						if (shapeList.length > 0) {
							...
							filterCustomArray[filterLayer] = [...filterCustomArray[filterLayer], ...shapeList];
						}
					...
					if (layer) {
						...
						if (featuresList && featuresList.length > 0) {
							const result = featuresList.filter(feature => {
								if (feature && feature.geometry && feature.geometry.type === 'MultiPolygon') {
									...
									for (let i = 0; i < shapeList.length; i++) {
										if (
											shapeList[i] &&
											(shapeList[i].type === 'MultiPolygon' || shapeList[i].geometry?.type === 'MultiPolygon')
										) {
							...
							let ids = result.map(function (feature) {
								if (defaultLayers.indexOf(filterLayer) > -1) {
			...
			if (stateNav.filterBasin && stateNav.filterBasin.length > 0) {
				...
				if (stateNav.basinName) {
					filterCustomArray['basin'] = ['match', ['get', 'NAME'], stateNav.basinName, true, false];
			...
			if (isFilterSet) {
				...
				if (filterCustomArray['wellpoints']) {
					map.setFilter('wellpoints', [
						...filterArray,
						['any', ['within', mergeIntoMultiPolygon(filterCustomArray['wellpoints'])], intersectingWellLinesFilter],
					]);
				} else if (Object.keys(filterCustomArray).length > 0) {
					map.setFilter('wellpoints', ['match', ['get', 'id'], '-1', true, false]);
				} else {
				...
				if (filterCustomArray['welllines']) {
					map.setFilter('welllines', [
						...filterArray,
						['any', ['within', mergeIntoMultiPolygon(filterCustomArray['welllines'])], intersectingWellLinesFilter],
					]);
				} else if (Object.keys(filterCustomArray).length > 0) {
				...
				if (filterCustomArray['wellpermitlines']) {
					map.setFilter('wellpermitlines', [
						...
						[
							'any',
							['within', mergeIntoMultiPolygon(filterCustomArray['wellpermitlines'])],
							intersectingWellPermitLinesFilter,
						],
					]);
				} else if (Object.keys(filterCustomArray).length > 0) {
				...
				filterLayers.forEach(filterLayer => {
					...
					if (filterCustomArray[filterLayer]) {
						...
						} else if (defaultLayers.indexOf(filterLayer) > -1) {
							map.setFilter(filterLayer, [
								'match',
								['get', 'shapeLabel'],
				...
				map.setFilter('wellpoints', ['all', ['match', ['geometry-type'], ['Point'], true, false]]);
				...
				map.setFilter('wellsHeatmapLast12', ['>', ['get', 'lastTwelveMonthBOE'], 0]);
				map.setFilter('wellsHeatmapLast12Oil', ['>', ['get', 'lastTwelveMonthOil'], 0]);
				map.setFilter('wellsHeatmapLast12Gas', ['>', ['get', 'lastTwelveMonthGas'], 0]);
	...
	const createPopUp = useCallback(
		currentFeature => {
			...
			setStateApp(state => ({
				...
				popupOpen: true,
				expandedCard:
					stateApp.activateWellDetailsFromTable || (currentFeature.id && currentFeature.id === paramId) ? true : false,
			}));
	...
	useEffect(() => {
		(async () => {
			if (
				...
				let currentFeature = features.find(element => element.properties.Id.toLowerCase() == stateApp.selectedPermitId);
	...
	useEffect(() => {
		if (map) {
			...
			for (let i = 0; i < featuresList.length; i++) {
				...
				map.setFeatureState({ source: 'abstract_geo_source', id: id }, { click: stateApp.filterSelectAllAbstract });
			}
		}
			...
			const LandClickListener = e => {
				...
				if (window.event.ctrlKey || window.event.metaKey || stateApp.multiSelectLandGrids) {
					if (featureState && featureState.click) {
						// Unselect feature
						map.setFeatureState({ source: 'abstract_geo_source', id: currentFeature.id }, { click: false });
			...
			map.on('mousemove', 'abstract_geo_fill_layer', function (e) {
				if (e.features.length > 0) {
					if (hoveredAbstractId) {
						map.setFeatureState({ source: 'abstract_geo_source', id: hoveredAbstractId }, { hover: false });
					}
					hoveredAbstractId = e.features[0].id;
					map.setFeatureState({ source: 'abstract_geo_source', id: hoveredAbstractId }, { hover: true });
				}
			});
	...
	useEffect(() => {
		...
		const initializeMap = ({ setMap, mapEl, setStateApp, setDraw }) => {
			...
			newMap.on('load', function (e) {
				if (globalStateController.getValue('cypress')) window.mapRef = newMap;
	...
	useEffect(() => {
		...
		if (stateApp.parcelDetailCardOpen && stateApp.parcelDetailCardOpen === true && map) {
			// set and remove map marker
			if (stateApp.selectedParcel.feature) findBoundsMap([stateApp.selectedParcel.feature], map);
	...
	useEffect(() => {
		if (
			(!stateApp.selectedUserDefinedLayer && !stateApp.selectedShape && !stateApp.selectedParcel) ||
			stateApp.shapeEdit
		) {
	...
	return (
		<div className={classes.mapWrapper}>
			...
			<MapControlsProvider
				{...{ expandedPanel: expandedPanel, openSpeedDial: openSpeedDial }}
				showExpandableCard={showExpandableCard}
				expandedCard={stateApp.expandedCard}
			/>
			...
			{mapGridCardActivated && <MapGridCardProvider mapGridCardActivated={mapGridCardActivated} />}

			{stateApp.selectedWell !== null && showExpandableCard && stateApp.expandedCard && (
			...
			{stateApp.selectedShape?.shapeLabel && stateApp.expandedCard && !hideShape && (
				<div className={classes.draggable}>
					<ExpandableCardProvider
						expanded={true}
						handleCloseExpandableCard={handleCloseExpandableCard}
						component={<ShapeDetailCard type={stateApp.selectedShape.type}></ShapeDetailCard>}
						title={stateApp.selectedShape?.shapeLabel}
						subTitle={stateApp.selectedShape?.shapeSubtitle || stateApp.selectedShape?.unitInfo}
						parent="map"
						position="relative"
						cardTop={0}
						cardLeft={0}
						...
						targetLabel={stateApp.selectedShape.type}
						deleteCustomLayer={deleteCustomLayer}
					></ExpandableCardProvider>
				</div>
			)}
			...
			<Portal container={modalContainer.current}></Portal>
