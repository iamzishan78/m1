...
export default function ActivitiesList({ activityLog, user_id, viewAll, ...props }) {
	...
	sortedActivityLog = viewAll ? sortedActivityLog : sortedActivityLog.slice(0, 3); // only get latest 3
							...
							<div className={classes.timelineText}>
								<div onClick={() => props.updateActivity(activity)}>
									...
									<Typography variant="body2" className={classes.blue}>
										{activity.ownerName} ● {moment(activity.dateTime).format('MMMM D, YYYY hh:mm a')}
									</Typography>
								</div>
							</div>
						</TimelineContent>
