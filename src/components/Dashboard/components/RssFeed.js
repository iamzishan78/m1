				...
				{news.map(({ feed, article, source, image }, i) => {
					...
					return (
                      ...
                      />
									...
									<Typography component="a" href={article.link} variant="h1" className={classes.title} target="_blank">
										{truncate(article.title, 75)}
									</Typography>
									<Typography className={classes.content}>{truncate(cleanedText(article.content), 150)}</Typography>
