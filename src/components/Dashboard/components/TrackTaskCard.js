...
const Title = ({ fromDate, setFromDate, toDate, setToDate, minDate }) => {
	...
	const getFilters = appliedFilters => {
		...
		if (appliedFilters) {
			...
			if (isValidDate(fromDate) && isValidDate(toDate)) {
				...
				const dateFilters = [
					getRangeFilters({ dateTime: { from: fromISO, to: toISO } }, 'simple'),
					getRangeFilters({ endDateTime: { from: fromISO, to: toISO } }, 'simple'),
				];
...
function TaskFilters({ fromDate, setFromDate, toDate, setToDate, minDate }) {
	const classes = useStyles();
