export const ProfileTabReducer = (state, action) => {
	switch (action.type) {
		...
		case 'isSameFromAbove':
			const employerAddress = !state.isSameFromAbove
...
export const InvestingPreFerencesReducers = (state, action) => {
	switch (action.type) {
		...
		case 'assetType':
			const InvestingInterestsTemp =
				state.investingPreferences.length > 0
					? state.investingPreferences.InvestingInterests
					: Object.assign([], state.investingPreferences.InvestingInterests);
			const assetTemp = typeof InvestingInterestsTemp.assetType === 'undefined' ? [] : InvestingInterestsTemp.assetType;
		...
		case 'vehicles':
			const InvestingInterestsVehiclesTemp =
				state.investingPreferences.length > 0
					? state.investingPreferences.InvestingInterests
					: Object.assign([], state.investingPreferences.InvestingInterests);
			const vehiclesTemp =
				typeof InvestingInterestsVehiclesTemp.vehicles === 'undefined' ? [] : InvestingInterestsVehiclesTemp.vehicles;
		...
		case 'objectives':
			const InvestingInterestsObjectivesTemp =
				state.investingPreferences.length > 0
					? state.investingPreferences.InvestingInterests
					: Object.assign([], state.investingPreferences.InvestingInterests);
			const objectivesTemp =
				typeof InvestingInterestsObjectivesTemp.objectives === 'undefined'
					? []
