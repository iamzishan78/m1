...
const InvestingEntities = () => {
	...
	const handleInputChange = (event, index) => {
		...
		switch (name) {
			...
			case 'postalCode':
			case 'country':
				tempForm.current[index] = {
					...tempForm.current[index],
					mailingInformation: {
						...tempForm.current[index].mailingInformation,
						[name]: value,
					},
				};
				break;
					...
					<Grid item sm={12}>
						<FormControl style={{ width: '100%', padding: 10 }}>
							<Grid container spacing={2}>
								<Grid item md={12}>
									<FormLabel>
										<strong>Entity Information ({index + 1}) </strong>
									</FormLabel>
									<Tooltip placement="top" title="Discard Entity">
										<IconButton variant="contained" onClick={() => discardEntity(index)}>
										...
										<Grid container spacing={1}>
											{element.entityMembers.length > 0 ? (
												element.entityMembers.map((entity, key) => {
													return (
														<Grid item md={12} key={key}>
															<Typography>
																({key + 1}) {entity.firstname} {entity.lastname}, {entity.email}
															</Typography>
														</Grid>
													);
												})
												...
												<Typography variant="caption">
													{' '}
													No entity members listed. Fill up the following fields (left) to add a member.{' '}
												</Typography>
											)}
										</Grid>
									</CardContent>
						...
						<Accordion className={classes.panels}>
							...
							<AccordionDetails>
								<Grid container spacing={2}>
									{entityContainer}
									<Grid item sm={6}>
										<Button variant="contained" fullWidth onClick={addNewEntity} disableElevation>
											<AddIcon style={{ margin: '2px' }} />
											Add Entity
										</Button>
									</Grid>
									<Grid item sm={6}>
										<Button variant="contained" fullWidth onClick={saveEntity} disableElevation>
											Save Entities
										</Button>
									</Grid>
								</Grid>
