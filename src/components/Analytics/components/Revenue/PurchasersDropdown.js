...
const PurchasersDropdown = ({
	...
	const [value, setValue] = useState('All Purchasers');
	...
	useEffect(() => {
		const filters = esFilters.filter(filter => !['purchaser.name.keyword'].includes(filter.field));
		...
		<Grid container direction="row" display="flex" justify="space-between" className={classes.actionBar}>
			<Grid item xs={strechedWidth ? true : fullWidth ? 7 : 3} md={strechedWidth ? true : fullWidth ? 7 : 3}>
				<FormControl variant="outlined" fullWidth className={classes.formControl}>
					<InputLabel id="select-outlined-label">Purchaser</InputLabel>
					<Select
						...
						onChange={e => {
							...
							if (e.target.value === 'All Purchasers') {
								setESFilters(esFilters.filter(filter => !['purchaser.name.keyword'].includes(filter.field)));
							} else {
								setESFilters([
									...esFilters.filter(filter => !['purchaser.name.keyword'].includes(filter.field)),
									{ field: 'purchaser.name.keyword', value: e.target.value },
								]);
							}
						}}
					>
...
export default PurchasersDropdown;
