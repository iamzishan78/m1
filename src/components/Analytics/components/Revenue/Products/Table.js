...
export default function OilTable({ monthsInterval, netRevenueTotals, item, title, grossVolumeType = 'BBL', price }) {
	...
	const totalGrossVolume =
		item.data && Math.round(Object.values(item.data).reduce((prev, curr) => (curr += prev), 0) / price / 0.375, 0);
	const totalOwnerVolume =
		item.data && Math.round(Object.values(item.data).reduce((prev, curr) => (curr += prev), 0) / price, 0);
						...
						>
							...
							<TableBody>
								<TableRow className={classes.highlightedRows}>
									{monthsInterval.map(month => (
										<TableCell scope="row">
											{vf_number(Math.round(totalGrossVolume / monthsInterval.length, 0))}
										</TableCell>
									))}
								</TableRow>
								<TableRow>
									{monthsInterval.map(month => (
										<TableCell scope="row">
											{vf_number(Math.round(totalOwnerVolume / monthsInterval.length, 0))}
										</TableCell>
									))}
								</TableRow>
								<TableRow className={classes.highlightedRows}>
								...
								<TableRow className={classes.highlightedLessBordered}>
									{monthsInterval.map(month => (
										<TableCell scope="row">
											{vf_number(Math.round(totalOwnerNetRevenue / monthsInterval.length, 0))}
										</TableCell>
									))}
								</TableRow>
