...
export default function TitleOpinion() {
	...
	const { MORDefaultSections, MORDefaultColumns, runsheetDefaultSections, runsheetDefaultColumns } = stateTitleOpinion;
	...
	useEffect(() => {
		if (stateApp.selectedTitleOpinionId) {
			const { data, loading } = QueryTitleOpinion(stateApp.selectedTitleOpinionId);
	...
	/////////////////////////////
	useEffect(() => {}, [stateTitleOpinion.TOData]);
							...
							<Grid item xs={12} className={classes.saveButtonsBar}>
								<div>
									<div className={classes.divSaveButtons}>
										<Button className={`${classes.saveButton} ${classes.buttons}`} onClick={handleAdd} color="primary">
											Save
										</Button>
										<Button className={classes.buttons} onClick={handleCancel} color="primary">
											Cancel
										</Button>
									</div>
								</div>
