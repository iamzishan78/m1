                ...
                <Autocomplete
                    ...
                    onChange={(e, value2, reason) => {
                        ...
                        if(allFilters.length > 0 && reporting.key){
                            const previousReporting = JSON.parse(JSON.stringify(gridViews?.getGridViews?.gridViews.find(f => reporting.key === f.name)))
                            ...
                            for(let i=0; i<previousReportingFilters.length; i++){
                                const index = allFilters.findIndex(f => f.field === previousReportingFilters[i].field && f.value === previousReportingFilters[i].value)
