		...
		if (esFilter) {
			esFilter.forEach(filter => {
				if (typeof filter?.field === 'string') {
					if (!filterHistory[filter.field]) filters.push(filter);
					else {
					...
					if (!filterHistory[filter.field[0]]) filters.push({ field: filter.field[0], value: filter.value[0] });
					else {
	...
	const onChange = (filter, index, column, esKey) => {
		...
		if (GridViewModule) {
			...
			if (filter.length) {
				...
				if (Array.isArray(filterKey)) filters = filters.filter(filter => !filterKey.includes(filter.field));
				...
				{selectedTractTab
					? tractInterestFilterColumnsHeader.map((filterColumn, index) => {
							...
							if (gridViewFilters && typeof filterColumn?.filterKey === 'string') {
								...
								if (filterColumn.name === 'County') {
									const stateFilter = gridViewFilters.find(
					...
					: tractFilterColumnsHeader.map((filterColumn, index) => {
							...
							if (gridViewFilters && typeof filterColumn?.filterKey === 'string') {
								...
								if (filterColumn.name === 'County') {
									const stateFilter = gridViewFilters.find(
										filter => filter.field === 'shapeJson.properties.originalProperties.State.keyword'
									);
