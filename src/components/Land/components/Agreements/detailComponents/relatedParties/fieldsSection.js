			...
			<Grid container display="flex" direction="row">
						...
						>
							<Grid container className={customClasses.gridStyle}>
								...
								<Grid item xs={1} style={{ textAlign: 'left', paddingLeft: 10 }}>
									<Tooltip
										title={item?.comments !== 0 ? 'Comments' : 'Add Comments'}
										placement="top"
										...
										<Badge badgeContent={item?.comments ?? null} color="secondary">
											<IconButton
												...
												className={`${classes.icons} ${!item?.comments || item?.comments === 0 ? classes.noCommentsIcon : ''} ${
													openCommentsDialog?.targetSourceId &&
													openCommentsDialog?.targetSourceId === item.descriptorObject?._id
				...
				<Grid item>
					<Button
						...
						color="primary"
						id="addPartyButton"
						className={customClasses.addDataButton}
						startIcon={<AddIcon />}
						onClick={addNewParty}
					>
						Add Another Party
