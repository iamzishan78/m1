...
export default function LagalDescription({ uniObj }) {
  ...
  const [getParcelFilesCount, { data: parcelFilesData }] = useLazyQuery(GET_PARCELS_FILES_COUNT, { fetchPolicy: "cache-and-network" });
            ...
            {uniObj?._id && (
              <Grid item xs={12} style={{ padding: "35px 20px 0px 0px" }}>
                <RelatedDetailsDocumentTable
                  ...
                  documentSearchQuery={`shapes.shape:${uniObj?._id}`}
                />
              </Grid>
            )}
          </Grid>
