...
// Header Schemas
import TableHeader from "components/Table/constants/parcel-billing-parties-header-schema.js";

// Utilities
import { usetableStyles } from "./style";
import { AppContext } from "AppContext";
import { DrawerContext } from "../DrawerContext";
import { REMOVEBILLINGPAYMENT } from "graphQL/useMutationRemoveBillingPayment";
import DeleteConfirmationDialogContent from "components/Shared/M1nTable/components/SubComponents/DeleteConfirmationDialogContent";
import { detailCardController } from "hookstate/detailCardController";

function AgreementBillingPartiesTable(props) {
    ...
    const [removeBillingPayment] = useMutation(REMOVEBILLINGPAYMENT, {
        onCompleted: () => {
    ...
    const formatHits = (hits) => {
        hits = hits.map((hit) => {
            const formattedHit = {};
            const { billingParties } = hit;
            if (Array.isArray(billingParties) && billingParties.length > 0) {
                const billingParty = billingParties.find((payment) => payment.paymentId === paymentId);
                if (billingParty) {
                    formattedHit._id = hit._id;
                    formattedHit.name = billingParty?.name;
                    formattedHit.address = billingParty?.address;
                    formattedHit.allocation = billingParty?.allocation;
                    formattedHit.amount = billingParty?.amount ? `$${billingParty.amount}` : '';
                    formattedHit.status = billingParty?.status;
                }
            }
            return formattedHit;
        });
    ...
    useEffect(() => {
        if (moduleId)
            props.setTableMeta({
                ...
                addableName: "Payment",
                searchFields: [],
                filters: [{ field: "billingParties.paymentId", value: paymentId }],
...
export default React.memo(TableESHOC(AgreementBillingPartiesTable), deepEqualObjects);
