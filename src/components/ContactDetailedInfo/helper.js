...
const getCreateByRow = contactData => {
	...
	) : (contactData?.createBy && contactData?.createBy.name) || contactData?.createAt ? (
		`${contactData?.createBy && contactData?.createBy.name ? contactData?.createBy.name : ''}
      ${contactData?.createAt ? ' - ' + anyToDate(contactData?.createAt).toLocaleString() : ''}`
	) : (
		<p>Not Available</p>
	);
};

const getLastUpdatedByValue = contactData => {
	return (contactData?.lastUpdateBy && contactData?.lastUpdateBy.name) || contactData?.lastUpdateAt
		? `${contactData?.lastUpdateBy && contactData?.lastUpdateBy.name ? contactData?.lastUpdateBy.name : ''}
    ${contactData?.lastUpdateAt ? ' - ' + anyToDate(contactData?.lastUpdateAt).toLocaleString() : ''}`
		: null;
};
...
const getLastUpdateByRow = contactData => {
	return contactData?.lastUpdateBy && contactData?.lastUpdateBy.name === null ? (
		...
		(getLastUpdatedByValue(contactData) ?? <p>Not Available</p>)
	);
};
...
export const getBasicPurchaseInfoExpContent = contactData => {
	return {
		...
		'Phone 1 Last Seen': {
			data: {
				phone1LastSeen: contactData?.phone1LastSeen ? moment(contactData.phone1LastSeen).format('MM/DD/YYYY') : null,
			},
			linkType: LinkTypes.None,
		},
		'Phone 1 Status': {
		...
		'Phone 3 Last Seen': {
			data: {
				phone3LastSeen: contactData?.phone3LastSeen ? moment(contactData.phone3LastSeen).format('MM/DD/YYYY') : null,
			},
			linkType: LinkTypes.None,
		},
		'Phone 3 Status': {
		...
		'Phone 4 Last Seen': {
			data: {
				phone4LastSeen: contactData?.phone4LastSeen ? moment(contactData.phone4LastSeen).format('MM/DD/YYYY') : null,
			},
			linkType: LinkTypes.None,
		},
		'Phone 4 Status': {
		...
		'Email 1 Last Seen': {
			data: {
				email1LastSeen: contactData?.email1LastSeen ? moment(contactData.email1LastSeen).format('MM/DD/YYYY') : null,
			},
			linkType: LinkTypes.None,
		},
		'Email 2': {
		...
		'Email 2 Last Seen': {
			data: {
				email2LastSeen: contactData?.email2LastSeen ? moment(contactData.email2LastSeen).format('MM/DD/YYYY') : null,
			},
			linkType: LinkTypes.None,
		},
		'Email 3': {
		...
		'Email 3 Last Seen': {
			data: {
				email3LastSeen: contactData?.email3LastSeen ? moment(contactData.email3LastSeen).format('MM/DD/YYYY') : null,
			},
			linkType: LinkTypes.None,
		},
		'Email 4': {
		...
		'Email 4 Last Seen': {
			data: {
				email4LastSeen: contactData?.email4LastSeen ? moment(contactData.email4LastSeen).format('MM/DD/YYYY') : null,
			},
			linkType: LinkTypes.None,
		},
		'Email 5': {
		...
		'Email 5 Last Seen': {
			data: {
				email5LastSeen: contactData?.email5LastSeen ? moment(contactData.email5LastSeen).format('MM/DD/YYYY') : null,
			},
			linkType: LinkTypes.None,
		},
...
export const SUMMARY_FIELDS = contactData => {
	let nraSumKey = contactData?.contactInterests?.nraSum
			? 'contactInterests.nraSum'
			: 'evaluatedContactInterests.nraSum',
		...
		closedPriceSum = contactData?.contactInterests?.closedPriceSum
			? 'contactInterests.closedPriceSum'
			: 'evaluatedContactInterests.closedPriceSum'; // get closedPriceSum from  evaluatedContactInterests
	return [
		{
			label: 'Full Name',
			key: 'name',
