...
const Login = props => {
				...
				.then(tokenResponse => {
					...
					if (accountObj) {
						...
						if (
							...
							tokenResponse.idTokenClaims.tfp === b2cPolicies.forgotPassword
						) {
							...
							if (queryString.parse(props.location.search).id_token_hint) {
								request.extraQueryParameters = {
									id_token_hint: queryString.parse(props.location.search).id_token_hint,
								};
							}
				...
				.catch(error => {
					if (error.errorMessage && error.errorMessage.includes('AADB2C90118')) {
						stateApp.myMSALObj.loginRedirect({
							authority: stateApp.myMSALObj.config.auth.authority.replace(
								b2cPolicies.signIn,
								b2cPolicies.forgotPassword
							),
						});
	...
	const handleBypassAADSignIn = async (tenantName, updateTenantFlags, email) => {
		...
		setTimeout(async () => {
			try {
				const {
					data: { simpleBypassLogin: loginResp },
				} = await Api.mutate(SIMPLE_BYPASS_LOGIN_MUTATION, {
					email,
				});
	...
	async function finishAADAuth(accountObj) {
		...
		const userSettingsResp = await userSettings(
			mongoUser._id,
			authGraphQLResponse.authenticationToken,
			authGraphQLToken.idToken,
			'baseMap'
