...
function ContactWellInterestTable(props) {
	...
	const [getPaginatedContactWellInterests, { data: dataContactWells }] = useLazyQuery(
		PAGINATED_CONTACT_WELLINTERESTS_QUERY,
				...
				getPaginatedContactWellInterests({
					...
					variables: {
						...
						pagination: {
							...
							before: rows && tableState.page < meta.pageInd ? rows[0]?.cursor : null,
							after: rows && tableState.page > meta.pageInd ? rows[rows.length - 1]?.cursor : null,
						},
					},
				});
				break;
