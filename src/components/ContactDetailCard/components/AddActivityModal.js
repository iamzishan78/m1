...
function AddActivityModal(props) {
	...
	const updateActivity = () => {
		...
		const index =
			newActLog &&
			newActLog.findIndex(
				activity => activity.dateTime === selectedActivity.dateTime && activity.user_id === selectedActivity.user_id
			);
		if (index > -1) {
			newActLog[index] = {
				...selectedActivity,
				dateTime: typeof dateTime.toISOString === 'function' ? dateTime.toISOString() : dateTime,
				type: activityType,
				notes,
			};
