...
const AutoCompleteWithAddNew = ({ setValue, value, fieldKey, defaultOptions = [], ...other }) => {
	...
	const [getContactFieldOptions, { data: contactFieldOptions }] = useLazyQuery(GET_ES_FILTER_LIST, {
	...
	useEffect(() => {
		if (contactFieldOptions?.getESFilterList?.hits) {
			let filterData = contactFieldOptions.getESFilterList.hits.map(hit => hit.key);
