				...
				{props.isTransactPage && (
					<Grid item xs={12} style={{ minHeight: '35px' }}>
						<h4 style={{ margin: '0 0 8px 0', float: 'left' }}>
							Documents ({get(files, 'getFileDescriptors.length', 0)})
						</h4>
						<h4
										...
										>
											<div
												onClick={() => {
													if (
														...
														// Enable view doc for images and pdf
											...
											>
												{new RegExp(['jpg', 'jpeg', 'png', 'bmp'].join('|')).test(fileExtension) ? (
													...
													<div className={classes.forImageContainer}>{get_file_icon(fileExtension)}</div>
												)}
