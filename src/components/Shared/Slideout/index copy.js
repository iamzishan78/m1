...
import DealTasksDetails from 'components/Transact/components/DealTasksDetails';
...
import ExistingDeal from 'components/Transact/components/DealDialog/ExistingDeal';
...
function DilogContent(props) {
  ...
  const [addContact, { data: addContactData, loading: addContactLoading }] =
  ...
  const [upsertDealDescriptor, { loading: upsertDealDescriptorLoading }] =
    useMutation(UPSERTDEALDESCRIPTOR);
  const [removeDealDescriptor] = useMutation(REMOVEDEALDESCRIPTOR);
  const [updateStageDealDescriptor, { data: updatedStageDealDescriptor }] = useMutation(
    UPDATE_STAGE_DEAL_DESCRIPTOR
  );
  ...
  useEffect(() => {
    if (cData?.contact) {
      setNameAutValue(
        cData?.contact ? { name: cData.contact.name, _id: cData.contact._id } : {}
      );
    }
  }, [cData]);
  ...
  useEffect(() => {
    ...
    if (cardId && laneId && stateApp.dealDialog) {
      ...
      setReceivedDate(
        card.receivedDate ? moment.parseZone(card.receivedDate).format('yyyy-MM-DD') : ''
      );
      ...
      setCloseDate(
        card.closeDate ? moment.parseZone(card.closeDate).format('yyyy-MM-DD') : ''
      );
  ...
  useEffect(() => {
    if (
      ...
      setStateApp(state => {
        return {
          ...state,
          mapVars: stateApp?.activeDeal?.mapSettings?.mapDefaultPosition,
        };
      });
    }
  }, [mapSettings]);
    ...
    if (pipelineId && stageId && title && title.trim() !== '') {
      ...
      const deal = {
        ...
        bidDate:
          selectedBidDate && selectedBidDate !== ''
            ? new Date(`${selectedBidDate}T08:00`).toUTCString()
            : null,
        dueDate:
          selectedDueDate && selectedDueDate !== ''
            ? new Date(`${selectedDueDate}T08:00`).toUTCString()
            : null,
        closeDate:
          selectedCloseDate && selectedCloseDate !== ''
            ? new Date(`${selectedCloseDate}T08:00`).toUTCString()
            : null,
      ...
      if (cardId) {
        ...
        addDeal({
          variables: {
            ...variables,
            deal: { ...variables.deal, _id: get(stateTransact, 'dealToCreate._id') },
          },
          refetchQueries: [
            'getPipeline',
            'getContactDeals',
  ...
  const [viewFiles, { data: viewFileResult, loading: viewFileLoading }] = useLazyQuery(
    VIEWFILESQUERY,
    {
      fetchPolicy: 'no-cache',
    }
