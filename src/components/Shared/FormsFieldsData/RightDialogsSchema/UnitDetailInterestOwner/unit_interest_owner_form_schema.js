...
const unitInterestOwnerForm = ({ getValues, setValue, newOwner, metafields = [] }) => {
	...
	const formFields = [
		{
			...
			onBlur: value => {
				if (!sideDialogController('unitInterestDialog').getValue('showNetRoyaltyAcresRecalculate')) {
					...
					const calculatedNra = calculateStandardNraForUnit({
						uAcres,
						working_interest: parseFloat(value).toFixed(8),
						royalty_interest,
						orri,
						nri,
						workspaceSettings,
		...
		},
		{
			...
			isValueOverridden: value => {
				...
				sideDialogController('unitInterestDialog').updateState({
					showNetRoyaltyAcresRecalculate: isOverride,
		...
		},
		{
			...
			isValueOverridden: value => {
				...
				sideDialogController('unitInterestDialog').updateState({
					'showTargetPrice/NraRecalculate': isOverride,
					rerenderJson: isOverride,
				});
				return isOverride;
			},
		...
		},
		{
			...
			isValueOverridden: value => {
				...
				sideDialogController('unitInterestDialog').updateState({
					showTargetOfferRecalculate: isOverride,
					rerenderJson: isOverride,
				});
				return isOverride;
			},
		...
		},
		{
			...
			isValueOverridden: value => {
				...
				sideDialogController('unitInterestDialog').updateState({
					'showMaxPrice/NraRecalculate': isOverride,
					rerenderJson: isOverride,
				});
				return isOverride;
			},
		...
		},
		{
			...
			isValueOverridden: value => {
				...
				const isOverride = parseFloat(calculatedOfferPrice) !== parseFloat(value);
				sideDialogController('unitInterestDialog').updateState({
					showMaxOfferRecalculate: isOverride,
					rerenderJson: isOverride,
				});
				return isOverride;
			},
	...
	const customDataJson = metafields.map(field => ({
		label: field.label,
		name: field.esKey,
		type: field.type, // Custom field type key added
