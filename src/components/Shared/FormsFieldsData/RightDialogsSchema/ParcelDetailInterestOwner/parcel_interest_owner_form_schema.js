	...
	const formFields = [
		...
		},
		{
			...
			isValueOverridden: value => {
				...
				sideDialogController('tractInterestDialog').updateState({
					showTargetOfferPriceRecalculate: isOverride,
					rerenderJson: isOverride,
				});
				return isOverride;
			},
		...
		},
		{
			...
			isValueOverridden: value => {
				...
				const isOverride = safeParseFloat(calculatedOfferPrice) !== safeParseFloat(value);
				sideDialogController('tractInterestDialog').updateState({
					showMaxOfferPriceRecalculate: isOverride,
					rerenderJson: isOverride,
				});
				return isOverride;
			},
		...
		// Bonus payment field
		{
			...
			isValueOverridden: value => {
				...
				sideDialogController('tractInterestDialog').updateState({
					showBonusPaymentRecalculate: isOverride,
					rerenderJson: isOverride,
				});
				return isOverride;
			},
	...
	const otherFields = [
		{
			...
			getOptions: apiRes => {
				if (!apiRes?.data?.getESFilterList?.hits) {
					return [];
				}
