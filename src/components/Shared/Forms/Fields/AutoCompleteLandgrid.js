...
export const AutoCompleteLandgrid = React.memo(function AutoCompleteLandgrid({
	onChange,
	filterKey,
	type,
	extendSearchQuery,
	...
	useEffect(() => {
		...
		if (label === 'Range') {
			if (compoundValue) hits = hits.filter(hit => hit.key.includes(compoundValue));
			hits = uniqBy(
	...
	const handleChange = search => {
		setSearch(search);
		if (label !== 'State') getFiltersAction(search);
	};
