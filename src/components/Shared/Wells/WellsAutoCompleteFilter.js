...
function Search(props) {
	...
	const [searchOption] = React.useState('wells');
	const [options, setOptions] = React.useState([]);
	const [searchTop] = React.useState(5);
	...
	const handleChange = newValue => {
		...
		upsertWellDescriptor({
			...
			if (props.setRefetchData) props.setRefetchData(!props.refetchData);
		});
		handleClose();
	};
			...
			<>
				<Autocomplete
					...
					PopperComponent={props => {
						return (
							<Popper {...props} style={{ height: '2px' }} className={classes.popperClass} placement="bottom-start" />
						);
					}}
					renderInput={params => (
						<TextField
