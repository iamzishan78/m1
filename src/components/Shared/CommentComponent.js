...
export default function CommentComponent(props) {
	const { targetSourceId, commentsHeight, setNewCommentId } = props;
	const classes = useStyles({
		commentsHeight,
	...
	}, [targetSourceId, getCommentsByObjectId]);
	...
	}, [dataComments, props.activityLog, sortArrayBasedOnTs]);
	...
	useEffect(() => {
		...
		if (!targetSourceId && newlyAddedComment?.upsertComment?.comment) {
			...
			if (setNewCommentId) setNewCommentId(newlyAddedComment.upsertComment.comment._id);
			setCommentsArray(sortArrayBasedOnTs([...comments]));
		}
	}, [newlyAddedComment, commentsArray, targetSourceId, setCommentsArray, user, sortArrayBasedOnTs, setNewCommentId]);
	...
	const newCommentCleaner = value => {
		...
		return value.trim().endsWith('.') ? cleanedLines : `${cleanedLines}.`;
	};
	...
	const callToggleCommentReactionMutation = useCallback(
		comment => {
	...
	const pinnedCommentsJsx = React.useMemo(
		...
		[
			...
			profilesInfo,
			users,
			callToggleCommentReactionMutation,
			didILikedThisComment,
			classes,
			user._id,
		]
	);
