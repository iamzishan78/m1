...
const useStyles = makeStyles((theme) => ({
  ...
  input: {
    "& input": {
      ...
      backgroundColor: ({ showPlusAddIcon, disabled }) =>
        !showPlusAddIcon ? "" : (disabled ? "lightgray" : "#D5F4FF"),
...
export default function Tags(props) {
  ...
  const [getDealTagOptions, { data: dataDealTagOptions }] = useLazyQuery(
    GETDEALTAGOPTIONS, {
    fetchPolicy: "cache-and-network",
  });
  ...
  useEffect(() => {
    if (dataDealTagOptions && dataDealTagOptions.getDealTagOptions && tagsArray) {
      ...
      setDealTagOptions([...dataDealTagOptions.getDealTagOptions.map(tag=> ({ tag: tag.tag, value: tag.tag, _id: tag._id, color: tag.color}))]);
    }
  ...
  const handleChangeTags = (e, newTagsArray) => {
    ...
    if(!tag.color && typeof tag === "object")
  ...
  const cleanDropDownArray = () => {
    ...
    let cleanArray = dealTagOptions.filter(
  ...
  useEffect(() => {
    ...
    if (
      ...
      setAddInDropDown({ tag: `Add "${UpperAndCleanTagText(textValue)}"`, value: UpperAndCleanTagText(textValue), color: generateColor()});
    } else {
    ...
    <div id="taggerRoot" className={classes.rootDiv}>
      {!loadingTags ? (
            <Autocomplete
              ...
              options={AddingAddRowToDropDown().map(({tag, value, color}) => ({ tag: tag, value: value || tag, color: color}))}
                ...
                return value
                  .map((tag, index) => (
                    <Chip
                      ...
                      style={{ background: tag.color || "powderblue", color: getOppositeHexColor(tag.color || "powderblue"), fontWeight: 'bold' }}
                    />
                  ));
              }}
              renderInput={(params) => (
