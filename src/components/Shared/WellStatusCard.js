...
export default function WellStatusCard(props) {
	...
	const WellStatusIcon = () => {
		if (summary.WellStatus && summary.WellStatus.toUpperCase() == 'ACTIVE') {
			return <CheckCircleIcon fontSize="large" />;
		} else if (summary.WellStatus && summary.WellStatus.toUpperCase() == 'UNKNOWN') {
			...
			{summary && (
				<div className={classes.iconContainer}>
					...
					<Typography
						...
						variant="caption"
					>
						{summary.WellStatus ? summary.WellStatus.toUpperCase() : '--'}
					</Typography>
				</div>
			)}
		</div>
