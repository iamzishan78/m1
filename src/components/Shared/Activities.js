...
export default function Activities({ activityLog, user_id, ...props }) {
	...
	const getIcon = activityType => {
		...
		return (
			<Icon classes={{ root: classes.iconRoot }}>
				<img className={classes.imageIcon} src={require(`../Shared/svgIcons/${icon}.svg`)} alt={activityType} />
			</Icon>
		);
	};
	...
	const deleteActivity = act => {
		...
		const index =
			newActLog &&
			newActLog.findIndex(activity => activity.dateTime === act.dateTime && activity.user_id === act.user_id);
		if (index > -1) {
		...
		<Card className={classes.root} variant="outlined">
			...
			<CardActions>
				<Grid container justify="space-between">
					...
					<Grid item>
						<Typography variant="button" onClick={addActivity} gutterBottom style={{ cursor: 'pointer' }}>
							Add Activity
						</Typography>
					</Grid>
				</Grid>
			...
			<Timeline>
				...
				{sortedActivityLog.map((activity, i) => (
					<TimelineItem key={i} className={classes.timelineItemRight}>
						<TimelineSeparator>
							<TimelineDot variant="outlined">{getIcon(activity.type)}</TimelineDot>
							{i + 1 !== activityLog.length && <TimelineConnector />}
						</TimelineSeparator>
						<TimelineContent>
							<div className={classes.timelineText}>
								<Typography className={classes.itemHeading} variant="body1" onClick={() => updateActivity(activity)}>
									{activity.notes}
								</Typography>
								<Typography variant="body2" className={classes.blue}>
									{activity.fullname} – {moment(activity.dateTime).format('MMMM D, YYYY hh:mm a')} <br />
									<h5 className={classes.deleteLine} onClick={() => deleteActivity(activity)}>
										Delete
									</h5>
								</Typography>
							</div>
