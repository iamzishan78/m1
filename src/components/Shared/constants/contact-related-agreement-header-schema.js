		...
		options: {
			...
			customRender: (value, tableMeta, updateValue) => {
				...
				const row_line = Object.assign(
					{},
					...tableMeta.rowData.map((item, index) => ({ [ContactReleatedAgreementHeaderCells[index]?.name]: item }))
				);

				return (
					<Link
						to={`/land/agreement/details/${tableMeta.rowData[0]}?tenant=${tenantName}`}
						style={{ color: GlobalStyles.colors.lightBlue, textDecoration: 'none' }}
					>
						{splitNumber?.[0] ? `${splitNumber?.[0].trim()} - ${row_line.agreementName}` : row_line.agreementName}
					</Link>
				);
			},
		},
