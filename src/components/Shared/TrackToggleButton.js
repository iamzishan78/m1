...
export default function TrackToggleButton(props) {
    ...
    const [selected,setSelected] = useState(props.target ? props.target.IsTracked :false)
...
useEffect( () => {
  if(sourceVertex && targetVertex){
    if(!selected) {
      ...
      //remove edge
          dropGraphEdge({ 
            variables: { source:sourceVertex,target:targetVertex,relationshipLabel: 'tracks' },
        ...
        createGraphEdge({ variables: { 
          source:sourceVertex,target:targetVertex,relationshipLabel: 'tracks' },
          refetchQueries:['getVertexEdges'],
          awaitRefetchQueries:true
         });
  ...
  console.log('edge drop',dataDrop)
   ...
   // if (loading || loadingDrop) return <CircularProgress size={28} color="secondary"></CircularProgress>;
    ...
    return (
      ...
      >
        {loading || loadingDrop ? (<CircularProgress size={28} color="secondary"></CircularProgress>) :
        selected ? (
