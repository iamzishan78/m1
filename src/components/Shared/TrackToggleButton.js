...
const useStyles = makeStyles(theme => ({
	root: {
		...
		'&:hover': {
			backgroundColor: props => (props.dark ? '#dadbde88 !important' : '#031d40 !important'),
		},
	},
	...
	return (
		<Tooltip
			title={`${props.target.isTracked ? 'Untrack' : 'Track'}${
				props.targetLabel ? ' ' + props.targetLabel.charAt(0).toUpperCase() + props.targetLabel.slice(1) : ''
		...
		>
			...
			>
				{loading ? (
					<CircularProgress size={props.iconZiseSmall ? 18 : 28} color="secondary" />
				) : selected ? (
					<>
