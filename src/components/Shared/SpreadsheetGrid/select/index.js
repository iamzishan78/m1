...
class SpreadsheetGridSelect extends React.PureComponent {
	...
	renderHeader() {
		return (
			<div className={this.getHeaderClassName()}>
				<div className="SpreadsheetGridSelectValue">{this.getHeaderValue()}</div>
			</div>
		);
	}
	...
	render() {
		return <Dropdown header={this.renderHeader()} body={this.renderBody()} isOpen={this.state.isOpen} />;
	}
}
