...
function SubTable(props) {
	...
	const gridElement = value => {
		...
		return (
			<>
				<Typography noWrap variant="body2" className={classes.gridElementStyling}>
	...
	const openActionMenu = (event, rowIndex, user, tableMeta) => {
		const contactId =
			user.columnData.parent === 'Tract detail' || user.columnData.parent === 'Unit detail'
				? user.rowData[1]
				: user.rowData[0];
		event.stopPropagation();
		setUsermanagementSettings(
									...
									return (
										...
										>
											<Avatar
												...
												fgColor="#000"
												name={tableMeta.rowData[nameIndex].split(' ').splice(0, 2).join(' ')}
												size="35"
												round
											/>
											<p
											...
											<IconButton
												...
												onClick={e => {
													...
													if (value) {
														...
														props.parent === 'assocTaxRollInterests' &&
														(props.targetLabel === 'unit' ||
															props.targetLabel === 'campaignUnit' ||
															props.targetLabel === 'contactUnits')
													) {
										...
										<>
											<Tooltip title="Actions" placement="top" style={{ marginRight: '10px' }}>
												<IconButton
													...
													size={props.dense ? 'small' : 'medium'}
													onClick={e => {
														openActionMenu(e, tableMeta.rowIndex, tableMeta);
													}}
												>
					...
					case 'partyName1': {
						{
							column.options = {
								...
								customBodyRender: (value, tableMeta, updateValue) => {
									return (
										<>
											{tableMeta.rowData[6] && tableMeta.rowData[6].entityDetail
												? tableMeta.rowData[6].entityDetail?.name
												: null}
										</>
									);
								},
					...
					case 'partyName2': {
						{
							column.options = {
								...
								customBodyRender: (value, tableMeta, updateValue) => {
									return (
										<>
											{tableMeta.rowData[7] && tableMeta.rowData[7].entityDetail
												? tableMeta.rowData[7].entityDetail?.name
												: null}
										</>
									);
								},
											...
											<IconButton
												...
												onClick={e => {
													...
													if (
														...
														const row_line = Object.assign(
															{},
															...tableMeta.rowData.map((item, index) => ({ [props.columns[index]?.name]: item }))
														);
													...
													<ColumnWithLink
														...
														link={
															isSnapGrid && tableMeta.rowData[3]
																? `/map/${tableMeta.rowData[3].toLowerCase()}s/${row_line.agreementId || tableMeta.rowData[0]}`
																: `/land/agreement/details/${row_line.agreementId || tableMeta.rowData[0]}`
														}
													/>
													{/* <Box
                            onClick={(e) => {
					...
					case 'commentsCounter':
						{
							column.options = {
								...
								customBodyRender: (value, tableMeta, updateValue) => {
									...
									if (
										props.parent === 'assocTaxRollInterests' &&
										(props.targetLabel === 'unit' || props.targetLabel === 'campaignUnit')
									) {
										...
										>
											<Badge badgeContent={value.length > 0 ? value.length : null} color="secondary">
												<IconButton
													size={props.dense ? 'small' : 'medium'}
													color="primary"
													className={`${classes.icons} ${!value || value.length === 0 ? classes.noOwnersIcon : ''} ${colInd === tableMeta.columnIndex && rowInd === tableMeta.rowIndex
					...
					case 'isContact':
						{
							column.options = {
								...
								customBodyRender: (value, tableMeta, updateValue) => {
									...
									return (
										<Tooltip
											title={!value || value === 'false' ? 'Convert To Contact' : 'Contact Details'}
											placement="top"
										>
											<IconButton
												size={props.dense ? 'small' : 'medium'}
													...
													if (value && value !== 'false') {
														...
														if (props.targetLabel === 'owner') {
															handleExpandClick(
																...
																{
																	globalOwner:
																		props.parent === 'OwnersPerWell' ? tableMeta.rowData[2] : tableMeta.rowData[0],
															...
															handleExpandClick(
																...
																{
																	globalOwner:
																		props.parent === 'ownersPerParcel' ? tableMeta.rowData[9] : tableMeta.rowData[0],
																	entity: tableMeta.rowData[1],
																},
																'makeOwnerAContact'
															);
											...
											>
												{!value || value === 'false' ? (
													...
													<Link
														to={`/contact/details/${value}/?tenant=${window.sessionStorage.getItem('tenantName')}`}
										...
										<Tooltip title={value ? 'Owners' : 'Not Available'} placement="top" style={{ marginRight: '10px' }}>
											<Badge badgeContent={value ? value : null} color="secondary">
												<IconButton
													size={props.dense ? 'small' : 'medium'}
													color="primary"
													className={`${classes.icons} ${!value ? classes.noOwnersIcon : ''} ${colInd === tableMeta.columnIndex && rowInd === tableMeta.rowIndex
															? classes.iconSelected
															: ''
														}`}
													onClick={e => {
									...
									return (
										<div style={{ marginRight: '10px' }}>
											<Tooltip title={value && value[1] === 0 ? 'Add Tags' : 'Tags'} placement="top">
												<Badge
													id={id + targetSourceId + tableMeta.rowIndex}
													className={`${classes.TagSample} ${colInd === tableMeta.columnIndex && rowInd === tableMeta.rowIndex
															? classes.iconSelected
															: ''
														}`}
					...
					case 'fileName':
						{
							column.options = {
								...
								customBodyRender: (value, tableMeta, updateValue) => {
									...
									const fileExtension = row_line?.fileName
										?.split('.')
									[row_line?.fileName?.split('.').length - 1]?.toLowerCase();
									...
									<>
										...
										{!value && (
											<div
												...
												onMouseOver={() =>
													(document.getElementById(`alertTootip${tableMeta.rowIndex}`).style.display = 'block')
												}
												onMouseOut={() =>
													(document.getElementById(`alertTootip${tableMeta.rowIndex}`).style.display = 'none')
												}
								...
								return (
									<>
										...
										{props.parent === 'RevenuePropertiesTable' && (
											<>
												{value?.toLowerCase() === 'notinpay' && 'Not in Pay'}
												{value?.toLowerCase() === 'inpay' && 'In Pay'}
												{/* {value?.toLowerCase() === 'approved' && (
                          <div className="flex justifyCenter alignCenter success w-100">
                            <CheckCircle size={20} />
                          </div>
					...
					case 'unitStatus':
						column.options = {
							...
							customBodyRender: value => {
								return (
									<div style={{ width: 250 }}>
										{value ? <StackedBarChart data={value} hideLegends eachBarHeight={5} /> : <p>N/A</p>}
									</div>
								);
							},
						};
	...
	const options = {
		...
		rowsPerPageOptions:
			props.rows && props.rows.length > 25 ? [10, 25, 50, 100] : props.rows && props.rows.length > 10 ? [10, 25] : [],
		...
		customToolbarSelect:
			props.header === 'Interest Owners Tied to Contact'
				...
				: (selectedRows, displayData, setSelectedRow) => {
					...
					if (
						props.addAble?.type === 'TractInterests' &&
						(props.parent === 'assocTaxRollInterests' || props.parent === 'contactAssocTaxRollInterests')
					) {
						return (
							<div
								style={{
				...
				<>
					...
					>
						...
						{(props.addAble?.type === 'deals' ||
							...
							props.addAble?.type === 'ownerToUnit') && <ButtonDropDown options={options} />}
		...
		onRowClick: (rowData, { dataIndex, rowIndex }) => {
			...
			if (
				props.parent === 'assocTaxRollInterests' &&
				props.targetLabel === 'well' &&
				props.addAble?.type !== 'taxrollInterest'
			) {
				let card = { ...rows[dataIndex] };
				setStateApp(stateApp => ({
					...stateApp,
			...
			if (
				(props.parent === 'assocTaxRollInterests' && props.targetLabel === 'parcel') ||
		...
		onTableChange: (action, tableState) => {
			// reset selected rows
			if (
				['changeRowsPerPage', 'changePage', 'sort', 'search', 'onSearchClose', 'filterChange', 'resetFilters'].includes(
					action
				)
			) {
				setM1nSelectedRowsIndexes([]);
	...
	const getHeaders = () => {
		if (
			(props.header === 'Contacts' && props.addAble?.type === 'contact') ||
			props.header === 'Documents' ||
			props.header === 'Agreements'
		) {
	...
	return (
			...
			>
				<MUIDataTable
					...
					components={{
						...
						TableFilterList:
							props?.component?.TableFilterList ||
							((props.header === 'Tax Roll Ownership' || props.parent === 'potentialOwnersPerUnit') && !isSearchOpen
								? TableFilterList
								: null),
						icons: {
							FilterIcon,
        ...
        // examples would be grid tags or grid comments  */}
				...
				{openDialog === 'merge' && (
					<MergeContactDrawer onClose={handleCloseDialog} rows={expandedObject} setRows={setExpandedObject} />
				)}
						...
						>
							...
							{openDialog === 'deleteWellInterest' && (
								<DeleteConfirmationDialogContent
									...
									setM1nSelectedRowsIndexes={setM1nSelectedRowsIndexes}
								>
									{`Do you want to permanently delete the well${m1nSelectedRowsIds &&
											m1nSelectedRowsIds.length > 1 &&
											removeDuplicatesIds(m1nSelectedRowsIds).length > 1
											? 's'
											: ''
							...
							{openDialog === 'deleteContact' && (
								...
								>
									...
									{props.header === 'Contacts' &&
										`Do you want to delete the selected ${['campaignContact'].includes(props.addAble?.type) ? 'record' : 'contact'
										}${m1nSelectedRowsIds &&
											m1nSelectedRowsIds.length > 1 &&
											removeDuplicatesIds(m1nSelectedRowsIds).length > 1
											? 's'
				...
				{multipleExpandableCard && targetLabelToExpand == 'contact' && (
					...
					>
						<ExpandableCardProvider
							...
							noTrackAvailable={
								targetLabelToExpand === 'contact' || (!targetLabelToExpand && props.targetLabel === 'contact')
									? true
									: false
							}
						/>
