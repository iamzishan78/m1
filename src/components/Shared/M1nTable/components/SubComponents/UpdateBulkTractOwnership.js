...
export default function UpdateBulkTractOwnership({
	onClose,
	rows,
	setRows,
	showSuccessMessage,
	...
	const onAssign = () => {
		...
		if (nameAutValue._id !== null) fieldToUpdate = { name: nameAutValue.name, ownerEntity: nameAutValue._id };
		...
		<RightDialog open={true} width={'700px'}>
			...
			<DialogActions className={modalClass.actionButtons}>
				...
				<Button
					...
					style={
						nameAutValue && nameAutValue._id === null && !fieldKey ? {} : { backgroundColor: '#00abed', color: 'white' }
					}
					onClick={onAssign}
				>
					Update
...
const SelectedFieldTest = React.memo(
	({
		field,
		fieldKey,
		contactOwner,
