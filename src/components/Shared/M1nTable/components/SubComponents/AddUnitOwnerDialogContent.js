...
export default function AddUnitOwnerDialogContent({ selectedRow, setSelectedRow, uAcres, uUnitPricing, ...props }) {
  ...
  const calculateNRA = (interest1 = 0, interest2 = 0, interest3 = 0, interest4 = 0, unitAcres = 0, workspaceSettings = {}, gross_acers) => {
    if (!interest4 && (!interest1 && !interest2 && !interest3)) return null;

    let nra = parseFloat(unitAcres) * (parseFloat(interest1 || 0) + parseFloat(interest2 || 0) + parseFloat(interest3 || 0));

    if (interest4) nra = parseFloat(interest4 || 0) * parseFloat(gross_acers || 0)
