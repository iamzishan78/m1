...
function M1nTable(props) {
	...
	const [selectedYear, setSelectedYear] = useState(LOD_YEAR); // production selected year state

	// selectors
	const { searchloading, searchResultData, selectedOwnerWellIntsSummary } = useSelector(
	...
	useEffect(() => {
		if (
			...
			const parcelOwners = dataParcelOwners.parcelOwners.map(o => {
				let parcelOwner = { ...o };
				parcelOwner.cost_bearing_high_value = parcelOwner?.cost_bearing_high_value
					? vf_currency(parcelOwner.cost_bearing_high_value)
					: undefined;
				parcelOwner.cost_free_high_value = parcelOwner?.cost_free_high_value
					? vf_currency(parcelOwner.cost_free_high_value)
					: undefined;
				if (parcelOwner.qtr) {
			...
			if (tenantName === 'Providence') {
				...
				OwnersPerParcelColumns.splice(index, 0, {
		...
		<Container maxWidth={false} className={classes.container} id={props.id ? props.id : props.parent}>
			...
			<Table
				...
				shapeWellsPageProps={{
					getPaginatedShapeWells,
					shapeWellsCount:
						dataShapeWellsCount && dataShapeWellsCount.shapeWellsCount ? dataShapeWellsCount.shapeWellsCount : 0,
					setLoading,
				}}
				parent={props.parent}
