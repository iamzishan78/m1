...
export default function CheckboxList(props) {
	const { basinLayerColor, GLOUnitsColor, GLOLeasesColor } = useSelector(({ MainMap }) => MainMap);
							...
							<RootRef rootRef={provided.innerRef}>
								<List className={classes.list}>
									{stateApp.layers.map((layer, index) => {
										...
										if (layer.layerSettings && layer.layerSettings.showable && layer.identifier != 'Tracked Owners') {
											return (
												<Draggable key={labelId} draggableId={labelId} index={layer.position}>
													{(provided, snapshot) => (
														<Box borderColor={getLayerColor(layer)} {...defaultProps}>
															...
															>
																...
																<ListItemText
																	...
																	className={!ifLayerHaveData(layer) ? classes.disabledLayerTitle : ''}
																/>
																...
																{layer.layerSettings.interaction.interactionAble && (
																	...
																	>
																		<Checkbox
																			icon={
																				<CancelOutlinedIcon
																					htmlColor={!ifLayerHaveData(layer) ? 'rgb(127, 149, 199)' : '#12abe0'}
																				/>
																			}
																			checkedIcon={
																				<ClickIcon color={!ifLayerHaveData(layer) ? 'rgb(127, 149, 199)' : '#12abe0'} />
																			}
																			edge="start"
																...
																<FormControlLabel
																	control={
																		<Switch
																			...
																			checked={layer.layerSettings.visiable !== false}
