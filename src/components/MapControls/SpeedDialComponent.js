...
export function SpeedDialComponent(props) {
  ...
  const { mapControlsStateValues } = mapControlsController.useState(['openSpeedDial', 'selectedMapControl', 'selectedControl'], 'mapControlsStateValues');
    ...
    if (e && action) {
      ...
      if (action === "base" || action === "heatMaps" || action === "layer") {
        ...
        mapControlsController.updateState({
          ...
          expandedPanel: action === mapControlsController.getValue('selectedControl') && mapControlsController.getValue('expandedPanel') ? false : true,
          anchorEl: anchorEl,
        })
  ...
  return (
    <div>
      <SpeedDial
        ...
        icon={<MenuIcon fontSize="small" onClick={mapControlsController.toggleSpeedDial} className={classes.menuIcon} />}
        onOpen={handleOpen}
      ...
      >
        {actions.map((action) => {
          return <SpeedDialAction
            classes={{
              fab: action.action === mapControlsStateValues.selectedControl || (toggle3d && action.action === 'threed') ? classes.fabActivated : classes.fab,
            }}
            id={action.name}
            key={action.name}
            icon={action.icon}
