...
export default function AddUserGroupData(props) {
	...
	const { fileUploadedContent, fileUploadedOriginalContent, mapControlsStateValues } = mapControlsController.useState(
		['fileUploadedContent', 'fileUploadedOriginalContent'],
		'mapControlsStateValues'
	);
	const [isOpen, setIsOpen] = useState(true);
	const [inputFiles, setInputFiles] = useState(mapControlsStateValues.fileUploadedContent);
	...
	const handleApplyChanges = async () => {
		if (!groupName) {
			...
			const fileName =
				groupName.trim().toLowerCase().replace(' ', '_') + `.${getFileExtension(inputOriginalFile.fileName)}`;
			let originalFileId = '';
			let originalFile;
			...
			await addDataset({
				variables: {
					dataset: {
						...
						categories: layerNames.map((layerName, index) => ({
							name: layerName,
							layerGeometry: mapControlsStateValues.fileUploadedContent.featureTypes[index],
							layerShapeName: mapControlsStateValues.fileUploadedContent.fileNames?.[index],
						})),
