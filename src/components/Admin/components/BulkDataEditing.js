      ...
      {!failedJob ? (
        <>
          ...
          >
            {jobs.map((job) => (
              <FeatureFlag
                feature={FEATURES[job.featureFlag]}
                noCheck={!FEATURES[job.featureFlag]}
              >
                <MenuItem
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    if (jobResponseData?.getJobResponse?.uploadedData) {
      const data = get(jobResponseData, 'getJobResponse.uploadedData', []).map(d => {
        return {
          ...
          name: get(d, 'entityDetail.name', '') || get(d, 'entityDetail.firstName', '') || get(d, 'entityDetail.lastName', '')
        }
      })
