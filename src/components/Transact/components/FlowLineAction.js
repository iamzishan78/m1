...
const FlowLineAction = ({ title, onEdit, onDelete, onDuplicate }) => {
	...
	return (
		<>
			<Tooltip title={title}>
				<IconButton size="medium" style={ICON_STYLE} onClick={handleClick}>
					<ExpandMoreIcon />
				</IconButton>
			</Tooltip>
			<Menu keepMounted anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleClose}>
				<MenuItem disabled={EDIT_DISABLED} onClick={handleEditFlowline}>
					<ListItemIcon classes={{ root: classes.icon }}>
						<EditIcon fontSize="small" />
					</ListItemIcon>
					<Typography>Edit Flowline</Typography>
				</MenuItem>
				<MenuItem disabled={DUPLICATE_DISABLED} onClick={handleDuplicateFlowline}>
					<ListItemIcon classes={{ root: classes.icon }}>
						<FileCopyIcon fontSize="small" />
					</ListItemIcon>
					<Typography>Duplicate Flowline</Typography>
				</MenuItem>
				<MenuItem disabled={DELETE_DISABLED} onClick={handleDeleteFlowline}>
					<ListItemIcon classes={{ root: classes.icon }}>
