...
const syncPositions = pipeLineStages => {
	const updatedStages = [];

	// Reorder stages by syncing positions
	pipeLineStages.forEach(stg => {
		let position = stg.position;

		// Ensure the position is unique by incrementing until no conflict
		while (updatedStages.some(existingStage => existingStage.position === position)) {
			position++;
		}

		// Add stage with updated position to the result
		updatedStages.push({ ...stg, position });
	});

	// Ensure stages are in sequential order starting from 0
	updatedStages.sort((a, b) => a.position - b.position); // Sort by position
	updatedStages.forEach((stg, index) => {
		stg.position = index; // Reassign positions sequentially
	});
...
export default function LanesInfoPanel({
	...
	useEffect(() => {
		if (openPipeDialog && selectedPipe && openPipeDialog !== 'newPipe') {
			if (selectedPipe.stages) {
				const syncedStages = syncPositions(selectedPipe.stages);
																...
																<TableRow key={stage.position} ref={provided.innerRef} {...provided.draggableProps}>
																	...
																	<TableCell align="left">
																		<Autocomplete
																			...
																			options={
																				flowLineType === 'general' ? ['Open', 'Closed'] : ['Open', 'Won', 'Lost']
																			}
																			renderInput={params => (
																				<TextField
																					{...params}
																	...
																	<TableCell padding="checkbox">
																		<Tooltip title="Stage Details" placement="top">
																			<DetailsIcon
																				onClick={() => handleStagesSave(index)}
