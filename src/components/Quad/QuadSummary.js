  ...
  return (
    ...
    data && stateQuad.quadChart ? (
      ...
      <AppBar className={classes.tabContainer} position="static">
        ...
        >
          <Tab value={0} className = {classes.tab} label="CUM" onClick={ () => {handleChangeRange(0)}}/>
          <Tab value={1} className = {classes.tab} label="1 Mo." onClick={ () => {handleChangeRange(1)}} />
          <Tab value={2} className = {classes.tab} label="6 Mo." onClick={ () => {handleChangeRange(6)} }/>
          <Tab value={3} className = {classes.tab} label="12 Mo." onClick={ () => {handleChangeRange(12)} }/>
        </Tabs>
      </AppBar>
      ...
      {stateQuad.quadChart.map(tile => (
        <GridListTile cols={1} rows={1} key={tile.metric}>
          ...
          <CardContent>
            <Typography  align="center" variant="h4">{tile.metric.toUpperCase()}</Typography>
            <Typography align="center" variant="h5">{stateQuad.selectedRange === 12 ? tile.value12 
            : stateQuad.selectedRange === 6 ? tile.value6 
            : stateQuad.selectedRange === 1 ? tile.value1
            : stateQuad.selectedRange === 0 ? tile.cumulative:'--'}</Typography>
            <Typography align="center" variant="h6">{tile.units}</Typography>
    ...
    :(<Skeleton variant="rect" height={325}><Typography variant="button">Not Available</Typography></Skeleton>)
