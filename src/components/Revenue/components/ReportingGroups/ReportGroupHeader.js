...
export default function ReportGroupHeader({ esFilters, setESFilters, setFilterToggle }) {
  ...
  const handleAddUpdateDelete = useCallback((updatedConfig) => {
    const type = config.type || updatedConfig.type
    if (['update', 'delete'].includes(type)) {
      ...
      const isDeleted = type === 'update' ? false : true
      updateGridView({
        variables: {
          gridView: {
            _id: gridViewId,
        ...
        if (resp.data.updateGridView.success) {
          setReportingGroup(type === 'update' ? name : 'All Properties')
        }
