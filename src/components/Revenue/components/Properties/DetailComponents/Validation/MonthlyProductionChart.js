      ...
      wellData.forEach((data) => {
        ...
        if (data.well.productionData.length > 0) {
          ...
          if (filter[0]?.value?.lte) {
            pData = pData.filter(
              (d) => moment(moment(d.data.ReportDate).format("MM/DD/yyyy")) <= moment(moment(filter[0]?.value?.lte).format("MM/DD/yyyy"))
            );
          }
          if (filter[0]?.value?.gte) {
            pData = pData.filter(
              (d) => moment(moment(d.data.ReportDate).format("MM/DD/yyyy")) >= moment(moment(filter[0]?.value?.gte).format("MM/DD/yyyy"))
            );
          }
