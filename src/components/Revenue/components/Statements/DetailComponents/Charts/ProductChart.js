	...
	useEffect(() => {
		...
		productSummaryDetails.forEach(data => {
			...
			donut.production.items.push({
				...
				[data.key.includes('NGL') ? 'NGL' : data.key]: vf_number(
					Number(removeCommasFromString(data.grsProd)).toFixed(2)
				),
			});
	...
	useEffect(() => {
		...
		chart.legend.labels.template.adapter.add('textOutput', (text, target) => {
			if (target.dataItem && target.dataItem.dataContext) {
				const rawValue = target.dataItem.values.value.value || 0;
				const value = rawValue === 0 ? '$0' : vf_currency_to_fixed(rawValue, 2);
				const percent = roundToTwoDecimalPlaces(
					target.dataItem.values.value.percent !== undefined ? target.dataItem.values.value.percent : 0
				);
	...
	return (
		<div className={classes.graphCard}>
			...
			<span style={{ height: '100%', display: mode === 'production' ? 'flex' : 'block' }}>
				{mode === 'production' ? (
					<div id={'bar-chart'} style={{ paddingTop: '40px', height: '80%', width: '55%' }} />
				) : (
					<div id={'product-donut'} style={{ height: '80%' }} />
				)}
