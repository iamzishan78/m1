...
export default function ActivityForm({ setSelectedActivityId }) {
    ...
    useEffect(() => {
        if (selectedActivity) {
            ...
            dealId.set(selectedActivity.dealId);
            ...
            status.set(selectedActivity.isClosed);
            setNameAutValue({
    ...
    useEffect(() => {
        if (formMode.get()) {
            if (formMode.get() === "update") {
                if (!selectedActivity) addActivity();
                else updateActivity();
    ...
    return (
        <div className={classes.inputFieldRoot}>
            ...
            <SimpleTextField title="Description" value={activityName} setValue={(value) => slidoutStateController.updateTitle(value)} />
            <SingleSelectField
                title="Type"
            ...
            <SingleSelectField
                ...
                onChange={(value) => { outcome.set(value) }}
            />
            <FormControl variant="outlined" fullWidth size="small">
                <Grid container className={classes.gridStyle}>
                    <DateField title="Start Date" date={startDate.get()} setDate={(value) => startDate.set(value)} />
                    <DateField title="End Date" date={endDate.get()} setDate={(value) => endDate.set(value)} />
                </Grid>
            </FormControl>

            <OwnerField title="Owner" users={users} setOwnerId={(value) => {
            ...
            <SimpleTextField title="Created By" disabled={true} value={selectedActivity?.creator?.name} setValue={(value) => slidoutStateController.updateTitle(value)} />
