				...
				<div className={props.showSummary ? classes.tableSize : classes.tableSize2}>
					<Table aria-label="simple table" className={classes.table}>
						<TableBody>
							...
							{stimulationData !== null &&
								stimulationData.length > 0 &&
								stimulationData.map((row, index) => (
									<TableRow key={index}>
										<TableCell>{moment(row.StimDate).format('MM-DD-YYYY')}</TableCell>
										<TableCell>{moment(row.StartDate).format('MM-DD-YYYY')}</TableCell>
										<TableCell>{moment(row.EndDate).format('MM-DD-YYYY')}</TableCell>
										<TableCell>{row.ServiceCompany}</TableCell>
										<TableCell>{row.NumberOfStages}</TableCell>
										<TableCell>{formatValue(row.TotalBaseWaterVolumeGallons)}</TableCell>
										<TableCell>{formatValue(row.TotalBaseNonWaterVolumeGallons)}</TableCell>
										<TableCell>{formatValue(row.TotalFracFluidVolume)}</TableCell>
										<TableCell>{formatValue(row.TotalBaseWaterMass)}</TableCell>
										<TableCell>{formatValue(row.TotalProppantMass)}</TableCell>
										<TableCell>{formatValue(row.TotalFracFluidMass)}</TableCell>
										<TableCell>{row.FluidType}</TableCell>
										<TableCell>{row.PrimaryProppantMaterial}</TableCell>
										<TableCell>{row.CrosslinkFluid}</TableCell>
										<TableCell>{row.Surfactant}</TableCell>
										<TableCell>{row.ClayControlAgent}</TableCell>
										<TableCell>{row.AcidType}</TableCell>
										<TableCell>{formatValue(row.AcidVolume)}</TableCell>
					...
					<Typography color="textSecondary" align="center">
						{stimulationData !== null && stimulationData.length === 0 ? 'No Stimulation records available' : ''}
					</Typography>
				</div>
			) : (
