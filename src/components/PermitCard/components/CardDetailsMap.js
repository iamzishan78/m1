...
export default function CardDetailsMap() {
	...
	useEffect(() => {
		map.jumpTo({
			center: [stateValues.selectedWell.longitude, stateValues.selectedWell.latitude],
			zoom: 16,
			speed: 0.4,
			bearing: -10,
			// flyTo v
	...
	useEffect(() => {
		if (mapStyles.length > 0) {
			...
			const initializeMap = ({ setMap, mapEl, setStateApp }) => {
				...
				if (stateValues.selectedWell && stateValues.selectedWell.longitude && stateValues.selectedWell.latitude)
				...
				if (stateValues.selectedWell && stateValues.selectedWell.longitude && stateValues.selectedWell.latitude)
					new mapboxgl.Marker(el)
						.setLngLat([stateValues.selectedWell.longitude, stateValues.selectedWell.latitude])
						.addTo(newMap);
