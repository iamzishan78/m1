...
const DocumentContextProvider = props => {
	...
	//Queries
	const [getWellsFromDocument, { data: wellsFromDocument, loading: getWellsLoading }] = useLazyQuery(
		GETWELLSFROMDOCUMENTS,
		{
	...
	const [getContactsFromDocument, { data: contactsFromDocument, loading: getContactsLoading }] = useLazyQuery(
		GETCONTACTSFROMDOCUMENTS,
		{
			fetchPolicy: 'cache-and-network',
			nextFetchPolicy: 'cache-first',
		}
	...
	const [getAgreementsFromDocument, { data: agreementsFromDocument, loading: getAgreementsLoading }] = useLazyQuery(
		GET_AGREEMENTS_FROM_DOCUMENTS,
		{
			fetchPolicy: 'cache-and-network',
			nextFetchPolicy: 'cache-first',
		}
	...
	const [getChecksFromDocument, { data: checksFromDocument, loading: getChecksLoading }] = useLazyQuery(
		GET_CHECKS_FROM_DOCUMENT,
		{
			fetchPolicy: 'cache-and-network',
			nextFetchPolicy: 'cache-first',
		}
	...
	const [getPropertiesFromDocument, { data: propertiesFromDocument, loading: getPropertiesLoading }] = useLazyQuery(
		GET_PROPERTIES_FROM_DOCUMENT,
		{
			fetchPolicy: 'cache-and-network',
			nextFetchPolicy: 'cache-first',
		}
	...
	useEffect(() => {
		const agreementDescriptor = agreementsFromDocument?.getAgreementDescriptors[0];
	...
	return <DocumentContext.Provider value={contextValue}>{props.children}</DocumentContext.Provider>;
};
