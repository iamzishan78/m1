...
export default function AssociatedAgreements() {
  ...
  const [addAgreementToFileDescriptor, { loading: addAgreementsLoading }] =
    useMutation(ADD_AGREEMENT_TO_FILE_DESCRIPTOR);

  // Mutations
  const [deleteAgreementFromDescriptor, { loading: deleteAgreementLoading }] =
    useMutation(DELETE_AGREEMENT_FROM_FILE_DESCRIPTOR, {
      onCompleted: () =>
        getAgreementsFromDocument({
          variables: {
