...
export default function AssociatedProperties() {
  ...
  const [addPropertyToFileDescriptor, { loading: addPropertiesLoading }] =
    useMutation(ADD_PROPERTY_TO_FILE_DESCRIPTOR);

  // Mutations
  const [deletePropertyFromDescriptor, { loading: deletePropertyLoading }] =
    useMutation(DELETE_PROPERTY_FROM_FILE_DESCRIPTOR, {
      onCompleted: () =>
        getPropertiesFromDocument({
          variables: {
  ...
  const searchExistingProperties = (value) => {
    ...
    if (value !== "") {
      const searchedProperties = existingProperties.filter((property) =>
  ...
  return (
    <DocumentAssociation
      ...
      onSearchBlur={() => {
        ...
        setProperties(
          propertiesFromDocument?.getPropertyDescriptors?.properties
        );
      }}
