...
export default function OwnerDetailsCardMap(props) {
	...
	useEffect(() => {
		if (map && dataWells && dataWells.wells && dataWells.wells.results && dataWells.wells.results.length > 0) {
			...
			var bbox = [
				[Math.min(...layerData.map(data => data.longitude)), Math.min(...layerData.map(data => data.latitude))],
				[Math.max(...layerData.map(data => data.longitude)), Math.max(...layerData.map(data => data.latitude))],
			];
	...
	return (
		<div className={classes.MSWrapper}>
			<div className={classes.map} ref={mapEl} id={`ownerDetailsCardMap${uid()}`}>
