...
export default function ShapeCardProvider(props) {
  ...
  return (
    <>
      {
        popupVals?.selectedShape?.type === 'unit' && <UnitCard
          closeCard={handleCloseCard}
          selectedShape={popupVals.selectedShape}
        />
      }
      {
        popupVals.selectedParcel?.shapeLabel &&
        (
          <ParcelCard
            selectedParcel={popupVals.selectedParcel}
          />
        )

      }
      {
        agreementLayers.includes(popupVals?.selectedShape?.type) && <AgreementCard
          closeCard={handleCloseCard}
          selectedShape={popupVals.selectedShape}
        />
      }
