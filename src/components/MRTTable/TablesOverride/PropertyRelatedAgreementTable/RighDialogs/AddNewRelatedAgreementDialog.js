...
const AddNewRelatedAgreementDialog = ({ customLayerId, handleClose, parentType, relatedAgreement, menuComponent }) => {
		...
		UPSERT_RELATED_AGREEMENT_DESSCRIPTOR,
		{
			...
			onCompleted: data => {
				if (data.upsertRelatedAgreementDescriptor.success) {
					...
					Loader.successToast('adding', 'Related Agreement created successful');
				}
			},
		}
	);
	...
	const selectedAgreement = useMemo(
		() => get(agreement, 'customLayer.shapeJson.properties') || relatedAgreement,
		[agreement, relatedAgreement]
	);
	...
	const addNewRelatedAgreement = () => {
		...
		Loader.createToast('adding', 'Related Agreement creation in Progress');
	...
	useEffect(() => {
		if (agreement) setEnableButton(true);
	}, [agreement]);
	...
	return (
		...
		>
			<div className={classes.dialogHeader}>
				...
				>
					{!relatedAgreement ? 'Add' : ''} Related Agreement
				</Typography>
