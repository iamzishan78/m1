...
export default function Information({ selectedDocument }) {
	...
	return (
		<>
			<ListContainer>
				<List>
					<StyledListItem>
						...
						<ListItemText
							primary={selectedDocument?.fileUploadedBy?.displayName || selectedDocument?.fileUploadedBy?.name}
						/>
					</StyledListItem>
					<StyledListItem>
			...
			<CommentComponent targetLabel={'file'} targetSourceId={selectedDocument?._id} />
		</>
	);
}
