...
const AssociatedWellsList = ({ title, selectedDocument }) => {
	// Initials
	...
	const deleteWell = async wellId => {
		await deleteWellFromDescriptor({
			variables: { descriptorId: selectedDocument?._id, wellGlobalId: wellId },
		});
	};
	...
	const getSelectedWell = async well => {
		...
		addWellToFileDescriptor({
			variables: { descriptorId: selectedDocument?._id, wellData: wellData },
		}).then(({ data }) => {
							...
							<div style={{ padding: '0px 0px 0px' }}>
								<ListItem key={index}>
									<a
										...
										href={`/map/wells/${well?.id.toUpperCase()}?breadcrumbs=Documents`}
										onClick={e => {
											e.preventDefault();
											goToWell(well);
										}}
...
export default function AssociatedWellsProvider({ title, selectedDocument }) {
	return (
		<DocumentContextProvider>
			<AssociatedWellsList title={title} selectedDocument={selectedDocument} />
		</DocumentContextProvider>
	);
}
