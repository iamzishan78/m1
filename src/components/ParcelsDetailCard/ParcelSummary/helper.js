...
export const getQtrFilterData = qtr => {
	...
	if (qtr && qtr[2] && qtr[3]) {
		...
		const lookup = quaterLookup.find(
			lookup => lookup.quater.length === 2 && lookup.quater[0] === qtr[0] && lookup.quater[1] === qtr[1]
		);
...
export const handleLayerChangeOnQtr = (stateApp, layerData, qtrQtr, qtr) => {
	const values = Object.keys(qtrQtr)
		.filter(key => qtrQtr[key])
