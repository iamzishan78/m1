...
describe('CheckDetails ESHOC Table', () => {
	it('checks that check details have correct property._id key', () => {
		cy.request({
			...
			const propertyIds = checkDetailsResponse.body.data.getDbData.hits.map(checkDetail => checkDetail.property._id);
			...
			uniquePropertyIds.map(propertyId => {
				cy.request({
					...
					cy.wrap(getPropertyResponse.body.data.getProperty).should('have.property', 'success', true);
				});
			});
		});
	});
