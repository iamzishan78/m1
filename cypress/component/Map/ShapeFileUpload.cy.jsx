...
const getIsGeometryWithinBbox = selectedUserDefinedLayer => {
	...
	if (selectedUserDefinedLayer.geometry.type === 'MultiPolygon') {
		...
		isGeometryWithinBbox = turf.booleanWithin(selectedUserDefinedLayer.geometry, bboxPolygon);
	}
...
const dataset = {
	...
	fileName: fileName,
	types: ['Point', 'Point', 'LineString', 'Point', 'Point', 'LineString', 'Polygon', 'Polygon'],
	public: true,
...
describe('Map Component Shape File Upload', () => {
	...
	it('Shapefile grid flyto works', () => {
		...
		cy.wait(basic_timeouts.shorTimeout).then(() => {
			...
			const { selectedUserDefinedLayer } = popupController.getValues(['selectedShapeFile', 'selectedUserDefinedLayer']);
	...
	it('Does not delete the group when a sub dataset is deleted', () => {
		...
		cy.get(`[data-testid='source-${sourceName}']`, { timeout: longTimeout }).scrollIntoView().click();
