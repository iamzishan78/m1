...
Cypress.Commands.add('verifyShape', ({ customLayer, shapeType, close = true }) => {
	...
	switch (shapeType) {
		case 'unit':
			...
			cy.get('[data-testid="data-cell-Unit Name"]').contains(customLayer.shapeJson.properties.shapeLabel);
		...
		case 'parcel':
			...
			// Verifying that the shape's name is displayed in the data cell
			cy.get('[data-testid="data-cell-Tract Name"]').contains(customLayer.shapeJson.properties.shapeLabel);
		...
		case 'surface':
			...
			// Verifying that the shape's name is displayed in the data cell
			cy.get('[data-testid="data-cell-Agreement Name"]').contains(customLayer.shapeJson.properties.agreementName);
			...
			cy.interceptAndWait(
				...
				alias => {
					...
					cy.wait(alias, { timeout: basic_timeouts.longTimeout }).then(response => {
						...
						expect(isEqual(customLayer.shapeJson.geometry, expectedShape)).to.be.equal(true);
...
Cypress.Commands.add('mapRightClick', ({ x, y, groupName, shapeName, callback }) => {
	...
	cy.interceptAndWait(
		...
		alias => {
			...
			cy.get(`div[data-testid="layer-selection-popup"] div[data-testid="${groupName}-group"] span.MuiTypography-root`, {
...
Cypress.Commands.add('rightClickAndDeleteShape', ({ x, y, groupName, shapeName, shapeType, newCustomLayer }) => {
