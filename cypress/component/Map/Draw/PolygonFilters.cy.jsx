	...
	cy.interceptAndWait(
		...
		alias => {
			...
			cy.wait(alias, { timeout: basic_timeouts.longTimeout }).then(response => {
				...
				const geo_intersects = response.request.body.variables.filters.find(f => f.type === 'geo_intersects');
				...
				const currentPolygon = turf.polygon(drawController.getValue('currentFeature').geometry.coordinates);
				...
				var points = turf.points(responseHits.map(hit => hit.geoJSON.geometries[0].coordinates));
