...
const getSettiingsToUpdate = (layerToMove, beforeLayer) => {
  ...
  const beforeLayerIndex = layerSettings.findIndex(
    layer => layer.identifier === beforeLayer
  );
...
describe('Map Component Layer Order', () => {
      ...
      alias => {
        ...
        cy.wait(alias, { timeout: basic_timeouts.longTimeout })
          .then(response => {
            ...
            const unitIndex = layerSettings.findIndex(
              layer => layer.identifier === 'Units'
            );
            const tractIndex = layerSettings.findIndex(
              layer => layer.identifier === 'Parcels'
            );
  ...
  it('Should move Tract/Unit above Unit/Tract', () => {
    cy.log(
      `Should move ${isUnitOnTop ? 'Tract' : 'Unit'} above ${
        isUnitOnTop ? 'Unit' : 'Tract'
      }`
    );
    ...
    let settingsToUpdate = getSettiingsToUpdate(
      isUnitOnTop ? 'Parcels' : 'Units',
      isUnitOnTop ? 'Units' : 'Parcels'
    );
