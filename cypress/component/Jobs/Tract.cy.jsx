...
import { VERIFY_TRACTS_JOB } from 'graphQL/useMutationCypressVerifyTractsJob';
...
describe('BulkUpload Component Tract Upload', () => {
  ...
  beforeEach(() => {
    cy.viewport(1800, 1200).mount(
      <BulkUpload routes={[]} initialJobType="TRACTS" />
    );
  });
  ...
  it('Tracts Upload is verified', () => {
    ...
    cy.request({
      ...
      body: {
        ...
        query: VERIFY_TRACTS_JOB.loc.source.body, // GraphQL query
      },
      timeout: basic_timeouts.longTimeout, // Set timeout
    }).then((response) => {
      const res = response.body.data.verifyTractsJob; // Extract response data


      Cypress.log({
        name: 'Check Tract Added',
        message: res.data.isTractAdded,
      });

      Cypress.log({
        name: 'Check Tract Updated',
        message: res.data.isTractUpdated,
      });
