...
const checkPrimaryAddress = job => {
	cy.wrap(job.resultsPayload.datasets[0].exportResponse[0]['Primary Address']).should('exist').and('not.be.empty');
};

const checkPurchasedPhoneNumbers = job => {
	cy.wrap(job.resultsPayload.datasets[0].exportResponse[0]['Phone 1']).should('exist').and('not.be.empty');
	cy.wrap(job.resultsPayload.datasets[0].exportResponse[0]['Phone 2']).should('exist').and('not.be.empty');
	cy.wrap(job.resultsPayload.datasets[0].exportResponse[0]['Phone 3']).should('exist').and('not.be.empty');
	cy.wrap(job.resultsPayload.datasets[0].exportResponse[0]['Phone 4']).should('exist').and('not.be.empty');
	cy.wrap(job.resultsPayload.datasets[0].exportResponse[0]['Phone 5']).should('exist').and('not.be.empty');
};
...
describe('Contact Table', () => {
	...
	it('Campaign Name Bulk Update Works', () => {
		...
		cy.get('[aria-labelledby="alert-dialog-slide-title"] [data-testid="campaign-name-autocomplete"] input').clear();
