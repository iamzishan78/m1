...
describe("Campaign Detail Page", () => {
  ...
  it("change campaign name", () => {
    cy.interceptAndWait(
      ...
      (alias) => {
        cy.get(
          '.MuiFormControl-root[data-testid="campaign-name-text-field"] div textarea:first-child'
        )
          .click()
          .type("{selectall}")
          .clear()
        ...
        cy.wait(alias, { timeout: basic_timeouts.longTimeout }).then(
          (updateResponse) => {
            expect(
              updateResponse?.response?.body?.data?.upsertCampaign?.message
            ).to.eq("Campaign Name Changes and Job is created Successfullly");
