...
Cypress.Commands.add('mrtInvokeText', ({ selector, as, index = 0, rowIndex = 0 }) => {
	if (selector) {
		...
		cy.log(index);
		cy.get(`tr.MuiTableRow-root[data-index="${rowIndex}"] > td.MuiTableCell-root`).eq(index).invoke('text').as(as);
	}
});
...
Cypress.Commands.add(
	...
	({ selector, index = 0, type = 'string', sorting, responseHits }) => {
		...
		cy.mrtInvokeText({
			...
			as: 'nthText',
			index,
			rowIndex: responseHits.length > 10 ? 10 : responseHits.length - 1,
		});
			...
			cy.get('@nthText').then(secondText => {
				...
				switch (type) {
					case 'string':
						...
						expect(sorting === 'ascending' ? firstText <= secondText : secondText <= firstText).to.be.equal(true);
						break;
					...
					case 'number':
						...
						const firstNumber =
							parseFloat(firstText.replace(/,/g, '')) || (sorting === 'ascending' ? 0 : Number.MAX_SAFE_INTEGER);
						const lastNumber =
...
Cypress.Commands.add('mrtSort', ({ column, apiAlias = '@getESSimpleSearchApiByIndex', sorting = false, sortOrder }) => {
	let responseHits;
	cy.interceptAndWait(
		['getDbData'],
		alias => {
			cy.get('table > thead > tr > th.MuiTableCell-root.MuiTableCell-head').contains(column.name).click();
...
Cypress.Commands.add('mrtNonEmptyFilterOnColumn', ({ column }) => {
	...
	cy.interceptAndWait(['getDbData'], () => {
		cy.get(`[data-testid="sentinelStart"] + div ul li:nth-child(${column?.emptyFilterIndex || 8}):eq(1)`).click();
	});
});
		...
		.then(columOpton => {
			cy.interceptAndWait(
				...
				alias => {
					...
					cy.wait(alias, { timeout: basic_timeouts.longTimeout }).then(selectedRows => {
						const responseData = selectedRows?.response?.body?.data?.getDbData.hits;
...
Cypress.Commands.add('mrtApplySpecificFilter', ({ column, optioText, callback }) => {
	...
	cy.get(`.MuiFormControl-root[data-testid="mrt-grid-filter-text-field-${column.name}"]`)
		.click()
		.clear()
		.type(`${optioText}`);
...
Cypress.Commands.add('mrtExport', ({ columns }) => {
	...
	cy.interceptAndWait(
		...
		alias => {
			...
			cy.wait(alias, { timeout: basic_timeouts.longTimeout }).then(response => {
				const responseColumns = response.response.body.data.initializeExportJob.job.requestPayload.columns;

				expect(
					columns.every(column => responseColumns.some(responseColumn => responseColumn.label === column.name))
				).to.be.equal(true);
			});
		},
		{ wait: false }
...
Cypress.Commands.add('VerifyAuthInfoMRT', () => {
	cy.get('table thead th div.Mui-TableHeadCell-Content-Wrapper').contains('Created By').should('exist');

	cy.get('table thead th div.Mui-TableHeadCell-Content-Wrapper').contains('Created By').should('exist');
	cy.get('table thead th div.Mui-TableHeadCell-Content-Wrapper').contains('Created Date').should('exist');
	cy.get('table thead th div.Mui-TableHeadCell-Content-Wrapper').contains('Last Updated By').should('exist');
	cy.get('table thead th div.Mui-TableHeadCell-Content-Wrapper').contains('Last Updated Date').should('exist');
});
