...
describe('Unit Table', () => {
	...
	it('should check grid have M1neral System ID column', () => {
		...
		cy.get('ul.MuiList-root > li.MuiButtonBase-root').contains('M1neral System ID').click();
		cy.get('body').click();

		cy.get('table thead th div.Mui-TableHeadCell-Content-Wrapper').contains('M1neral System ID').should('exist');
	});
	...
	it('Open Bulk Update for Unit Grid', () => {
		...
		cy.get('input[type="checkbox"]')
			.not('[aria-label="Toggle select all"]')
			.each((checkbox, index) => {
				if (index < 5) {
		...
		cy.get('[data-testid="select-field-autocomplete"]', { timeout: 10000 }).should('be.visible').click({ force: true });
	...
	it('Tags Bulk Update Works', () => {
		...
		cy.get('[aria-labelledby="alert-dialog-slide-title"] [data-testid="tags-autocomplete"] input').clear();
