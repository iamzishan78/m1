...
Cypress.Commands.add('checkAndLogin', () => {
	...
	cy.get('body').then($body => {
		if ($body.find('#workSpaceSignin').length) {
			...
			cy.get('#organizationName').type(organization, { force: true });
			cy.contains('button', 'Continue').click({ force: true });

			cy.get('#username', { timeout: longTimeout }).should('be.visible').type(loginCredential.email);
...
Cypress.Commands.add(
	'interceptAndWait',
	(dataKeys, interceptionFunction, options = { wait: true }, url = baseUrls[workSpace]) => {
...
Cypress.Commands.add('deleteConfirmation', () => {
	cy.get('.MuiTypography-root', { timeout: longTimeout })
		.contains('Delete', { timeout: longTimeout })
		.scrollIntoView()
		.click({ force: true });
...
Cypress.Commands.add('gridSearch', (searchString, gridOperationName, searchId = null) => {
	...
	cy.get(searchId || commonSearchClass)
		.focus()
		.clear()
		.type(searchString);
...
Cypress.Commands.add('deleteTractAndVerify', tractName => {
	cy.verifyApiResponse('@getESSimpleSearchApiByIndex', { responseTimeout: longTimeout }).then(response => {
		...
		cy.wait(1000);
		cy.get('#AgreementOwnersTractsTable').scrollIntoView();
		cy.get('#AgreementOwnersTractsTable', { timeout: longTimeout })
			.scrollIntoView()
			.getTableCell('State', indexOfSampleContact)
			.scrollIntoView()
			.click();
...
Cypress.Commands.add('openAgreementUploader', breadCrumb => {
	...
	cy.get('.MuiTypography-root', { timeout: longTimeout })
		.contains('Agreement Upload (Agreement Header Info)', { timeout: longTimeout })
		.click();
...
Cypress.Commands.add('checkFieldsMapping', () => {
	...
	cy.get('#headerTable', { timeout: longTimeout })
		.should('be.visible')
		.find('tr')
		.then(row => {
