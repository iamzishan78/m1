	...
	it('passes', () => {
		...
		cy.verifyApiResponse('@getESSimpleSearchApi').then(() => {
			cy.getTableCell('Agreement', 1).then($tableCell => {
				...
				cy.wait(shorTimeout);
				cy.get('#header-tabs', { timeout: longTimeout }).should('be.visible').contains('Documents').click();
				cy.wait(4000);
				cy.get('button', { timeout: longTimeout }).contains('+ ADD RELATED DCMNT').click({ force: true });
				...
				cy.verifyApiResponse('@AddDescriptorFileApi').then(response => {
					...
					cy.wait(2000);
					cy.get('button', { timeout: shorTimeout }).contains('+ ADD RELATED DCMNT').click();
					cy.wait(2000);
					...
					cy.verifyApiResponse('@AddDescriptorFileApi').then(response2 => {
						...
						cy.verifyApiResponse('@getESSimpleSearchApi').then(() => {
							...
							cy.wait(shorTimeout);
							cy.get('#related-docs-div tbody tr:first-child td:first-child input').click({ force: true });
							cy.get('#related-docs-div tbody tr:nth-child(2) td:first-child input').click({ force: true });
