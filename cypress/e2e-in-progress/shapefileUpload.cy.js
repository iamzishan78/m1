	...
	it('passes', () => {
		...
		cy.verifyApiResponse('@getDatasetsApi', { responseTimeout: longTimeout }).then(response => {
			...
			if (hits.some(hit => hit.fileName === fileName)) {
				...
				cy.get("[id='more-horiz-cypressford']", { timeout: longTimeout })
					.scrollIntoView()
					.invoke('show')
					.click({ force: true });
				...
				cy.verifyApiResponse('@getDatasetsApi', { responseTimeout: longTimeout }).then(result => {
					const filesNames = result.response?.body?.data?.getDatasets.map(hit => hit.fileName);
			...
			cy.verifyApiResponse('@getDatasetsApi', { responseTimeout: longTimeout }).then(result => {
				...
				if (!filesNames.includes(fileName)) {
					cy.verifyApiResponse('@getDatasetsApi', { responseTimeout: longTimeout }).then(result2 => {
						...
						const filesNames2 = result2.response?.body?.data?.getDatasets.map(hit => hit.fileName);
				...
				cy.get('#layerSnapGrid', { timeout: extraTimeout })
					.should('be.visible')
					.getTableCell('geom', 1)
					.then($tableCell => {
