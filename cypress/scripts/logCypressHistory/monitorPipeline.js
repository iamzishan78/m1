...
const monitorPipeline = ({ getCypressProcess, specs }) => {
	...
	const intervalId = setInterval(async () => {
		...
		if (elapsedTime >= PIPELINE_WARNING_THRESHOLD && elapsedTime < MAX_PIPELINE_DURATION) {
			console.log('WARNING: Pipeline will exceed the maximum allowed time in less than 1 minute.');
			...
			if (cypressProcess) {
				...
				pipelineState = pipelineHistory.find(history => history.buildId === BUILD_ID)?.state;
