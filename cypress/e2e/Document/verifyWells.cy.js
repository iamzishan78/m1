...
describe('Verify Well Spec', () => {
    it('passes', async () => {
        ...
        let wellsResponse = await promisify(cy.wait('@getWellsFromDocumentApi'))
        ...
        wellsResponse = await promisify(cy.wait('@getWellsFromDocumentApi'))
        const wellsAfterDocDelete = wellsResponse.response.body?.data?.getWellDescriptors[0]?.wells
